...
- Safe evidence descriptor: The repo contains a deterministic internal eval
  ...
  and `getattr(obj, name)`, and must not be described as public benchmark
  proof or generalized hybrid-runtime support.
...
- When referencing runtime-backed work, say only that narrow internal evidence
  ...
  `REFLECTIVE_BUILTIN` / `hasattr(obj, name)` and `getattr(obj, name)` pilots.
