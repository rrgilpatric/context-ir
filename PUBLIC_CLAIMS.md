...
- Safe evidence descriptor: The repo contains a deterministic internal eval
  ...
  internal one-argument `vars(obj)` pilot. The three existing
  ...
  `oracle_signal_vars_probe_matrix` is limited to 1 task x 1 budget x 3
  providers at budget `220` with `lookup_outcome=returned_namespace`. These
...
- When referencing runtime-backed work, say only that narrow internal evidence
  ...
  pilots, plus the current internal one-argument `vars(obj)` pilot. For
  ...
  provider/budget wording is 1 task x 1 budget x 3 providers at budget `220`.
