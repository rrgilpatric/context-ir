...
- Safe evidence descriptor: The repo contains a deterministic internal eval
  ...
  existing getattr-family pilot matrices
  ...
  2 budgets x 3 providers at budgets `100` and `220`, and must not be
  described as public benchmark proof, generalized `getattr` support, or
  generalized hybrid-runtime support.
...
- When referencing runtime-backed work, say only that narrow internal evidence
  ...
  pilots. For the three existing getattr-family matrices, the only accepted
