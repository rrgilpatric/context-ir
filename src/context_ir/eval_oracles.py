...
_LOCALS_RUNTIME_OBSERVATION_FILENAME = _RUNTIME_OBSERVATION_FILENAME
_METACLASS_BEHAVIOR_RUNTIME_OBSERVATION_FILENAME = _RUNTIME_OBSERVATION_FILENAME
_SETATTR_RUNTIME_OBSERVATION_FILENAME = _RUNTIME_OBSERVATION_FILENAME
_VARS_RUNTIME_OBSERVATION_FILENAME = _RUNTIME_OBSERVATION_FILENAME
_DELATTR_RUNTIME_OBSERVATION_FILENAME = _RUNTIME_OBSERVATION_FILENAME
...
_ALLOWED_GLOBALS_OBSERVATION_FIELDS = _ALLOWED_DYNAMIC_IMPORT_OBSERVATION_FIELDS
_ALLOWED_LOCALS_OBSERVATION_FIELDS = _ALLOWED_DYNAMIC_IMPORT_OBSERVATION_FIELDS
_ALLOWED_METACLASS_BEHAVIOR_OBSERVATION_FIELDS = (
    _ALLOWED_DYNAMIC_IMPORT_OBSERVATION_FIELDS
)
_ALLOWED_SETATTR_OBSERVATION_FIELDS = _ALLOWED_DYNAMIC_IMPORT_OBSERVATION_FIELDS
_ALLOWED_VARS_OBSERVATION_FIELDS = _ALLOWED_DYNAMIC_IMPORT_OBSERVATION_FIELDS
_ALLOWED_DELATTR_OBSERVATION_FIELDS = _ALLOWED_DYNAMIC_IMPORT_OBSERVATION_FIELDS
...
def setup_eval_oracle_task(
    ...
    dir_runtime_observations = load_fixture_dir_runtime_observations(
        root,
        semantic_program=program,
    )
...
def load_fixture_eval_runtime_observations(
    repo_root: Path | str,
    *,
    semantic_program: SemanticProgram | None = None,
) -> tuple[EvalRuntimeObservation, ...]:
    """Load fixture-local bounded ``eval(source)`` runtime observations."""
    root = Path(repo_root)
    observation_path = root / _EVAL_RUNTIME_OBSERVATION_FILENAME
    if not observation_path.is_file():
        return ()
    ...
    try:
        ...
        raise EvalOracleSchemaError(
            f"invalid eval runtime observation JSON in {observation_path}: {error}"
        ) from error
    _reject_generated_id_fields(raw, path="$")
    record = _expect_object(raw, path="$")
    _validate_allowed_fields(
    ...
    if not observation_records:
        raise EvalOracleSchemaError(
            "eval runtime observation file must contain at least one observation"
        )

    program = (
        semantic_program if semantic_program is not None else analyze_repository(root)
    )
    site_index = _eval_observation_site_index(program)
    return tuple(
...
def load_fixture_getattr_runtime_observations(
    repo_root: Path | str,
    *,
    semantic_program: SemanticProgram | None = None,
) -> tuple[GetattrRuntimeObservation, ...]:
    """Load fixture-local ``getattr`` runtime observations when present."""
    root = Path(repo_root)
    observation_path = root / _GETATTR_RUNTIME_OBSERVATION_FILENAME
    if not observation_path.is_file():
        return ()
    ...
    try:
        ...
        raise EvalOracleSchemaError(
            f"invalid getattr runtime observation JSON in {observation_path}: {error}"
        ) from error
    _reject_generated_id_fields(raw, path="$")
    record = _expect_object(raw, path="$")
    _validate_allowed_fields(
    ...
    if not observation_records:
        raise EvalOracleSchemaError(
            "getattr runtime observation file must contain at least one observation"
        )

    program = (
        semantic_program if semantic_program is not None else analyze_repository(root)
    )
    site_index = _getattr_observation_site_index(program)
    return tuple(
        _parse_getattr_runtime_observation(
            observation_record,
            path=f"$.getattr_runtime_observations[{index}]",
            site_index=site_index,
        )
        for index, observation_record in enumerate(observation_records)
    )
...
def load_fixture_dir_runtime_observations(
    repo_root: Path | str,
    *,
    semantic_program: SemanticProgram | None = None,
    ...
    try:
        ...
        raise EvalOracleSchemaError(
            f"invalid dir runtime observation JSON in {observation_path}: {error}"
        ) from error
    _reject_generated_id_fields(raw, path="$")
    record = _expect_object(raw, path="$")
    _validate_allowed_fields(
    ...
    if not observation_records:
        raise EvalOracleSchemaError(
            "dir runtime observation file must contain at least one observation"
        )

    program = (
        semantic_program if semantic_program is not None else analyze_repository(root)
    )
    site_index = _dir_observation_site_index(program)
    return tuple(
        _parse_dir_runtime_observation(
            observation_record,
            path=f"$.dir_runtime_observations[{index}]",
            site_index=site_index,
        )
        for index, observation_record in enumerate(observation_records)
    )
...
def load_fixture_vars_runtime_observations(
    repo_root: Path | str,
    *,
    semantic_program: SemanticProgram | None = None,
    ...
    try:
        ...
        raise EvalOracleSchemaError(
            f"invalid vars runtime observation JSON in {observation_path}: {error}"
        ) from error
    _reject_generated_id_fields(raw, path="$")
    record = _expect_object(raw, path="$")
    _validate_allowed_fields(
    ...
    if not observation_records:
        raise EvalOracleSchemaError(
            "vars runtime observation file must contain at least one observation"
        )

    program = (
        semantic_program if semantic_program is not None else analyze_repository(root)
    )
    site_index = _vars_observation_site_index(program)
    return tuple(
        _parse_vars_runtime_observation(
            observation_record,
            path=f"$.vars_runtime_observations[{index}]",
            site_index=site_index,
        )
        for index, observation_record in enumerate(observation_records)
    )
...
def load_fixture_globals_runtime_observations(
    repo_root: Path | str,
    *,
    semantic_program: SemanticProgram | None = None,
) -> tuple[GlobalsRuntimeObservation, ...]:
    """Load fixture-local ``globals()`` runtime observations when present."""
    root = Path(repo_root)
    observation_path = root / _GLOBALS_RUNTIME_OBSERVATION_FILENAME
    if not observation_path.is_file():
        return ()
    ...
    try:
        ...
        raise EvalOracleSchemaError(
            f"invalid globals runtime observation JSON in {observation_path}: {error}"
        ) from error
    _reject_generated_id_fields(raw, path="$")
    record = _expect_object(raw, path="$")
    _validate_allowed_fields(
    ...
    if not observation_records:
        raise EvalOracleSchemaError(
            "globals runtime observation file must contain at least one observation"
        )

    program = (
        semantic_program if semantic_program is not None else analyze_repository(root)
    )
    site_index = _globals_observation_site_index(program)
    return tuple(
        _parse_globals_runtime_observation(
            observation_record,
            path=f"$.globals_runtime_observations[{index}]",
            site_index=site_index,
        )
        for index, observation_record in enumerate(observation_records)
    )
...
def load_fixture_locals_runtime_observations(
    repo_root: Path | str,
    *,
    semantic_program: SemanticProgram | None = None,
) -> tuple[LocalsRuntimeObservation, ...]:
    """Load fixture-local ``locals()`` runtime observations when present."""
    root = Path(repo_root)
    observation_path = root / _LOCALS_RUNTIME_OBSERVATION_FILENAME
    if not observation_path.is_file():
        return ()
    ...
    try:
        ...
        raise EvalOracleSchemaError(
            f"invalid locals runtime observation JSON in {observation_path}: {error}"
        ) from error
    _reject_generated_id_fields(raw, path="$")
    record = _expect_object(raw, path="$")
    _validate_allowed_fields(
    ...
    if not observation_records:
        raise EvalOracleSchemaError(
            "locals runtime observation file must contain at least one observation"
        )

    program = (
        semantic_program if semantic_program is not None else analyze_repository(root)
    )
    site_index = _locals_observation_site_index(program)
    return tuple(
        _parse_locals_runtime_observation(
            observation_record,
            path=f"$.locals_runtime_observations[{index}]",
            site_index=site_index,
        )
        for index, observation_record in enumerate(observation_records)
    )
...
def load_fixture_setattr_runtime_observations(
    repo_root: Path | str,
    *,
    semantic_program: SemanticProgram | None = None,
) -> tuple[SetattrRuntimeObservation, ...]:
    """Load fixture-local ``setattr(obj, name, value)`` runtime observations."""
    root = Path(repo_root)
    observation_path = root / _SETATTR_RUNTIME_OBSERVATION_FILENAME
    if not observation_path.is_file():
        return ()
    ...
    try:
        ...
        raise EvalOracleSchemaError(
            f"invalid setattr runtime observation JSON in {observation_path}: {error}"
        ) from error
    _reject_generated_id_fields(raw, path="$")
    record = _expect_object(raw, path="$")
    _validate_allowed_fields(
    ...
    if not observation_records:
        raise EvalOracleSchemaError(
            "setattr runtime observation file must contain at least one observation"
        )

    program = (
        semantic_program if semantic_program is not None else analyze_repository(root)
    )
    site_index = _setattr_observation_site_index(program)
    return tuple(
        _parse_setattr_runtime_observation(
            observation_record,
            path=f"$.setattr_runtime_observations[{index}]",
            site_index=site_index,
        )
        for index, observation_record in enumerate(observation_records)
    )
...
def load_fixture_delattr_runtime_observations(
    repo_root: Path | str,
    *,
    semantic_program: SemanticProgram | None = None,
) -> tuple[DelattrRuntimeObservation, ...]:
    """Load fixture-local ``delattr(obj, name)`` runtime observations."""
    root = Path(repo_root)
    observation_path = root / _DELATTR_RUNTIME_OBSERVATION_FILENAME
    if not observation_path.is_file():
        return ()
    ...
    try:
        ...
        raise EvalOracleSchemaError(
            f"invalid delattr runtime observation JSON in {observation_path}: {error}"
        ) from error
    _reject_generated_id_fields(raw, path="$")
    record = _expect_object(raw, path="$")
    _validate_allowed_fields(
    ...
    if not observation_records:
        raise EvalOracleSchemaError(
            "delattr runtime observation file must contain at least one observation"
        )

    program = (
        semantic_program if semantic_program is not None else analyze_repository(root)
    )
    site_index = _delattr_observation_site_index(program)
    return tuple(
        _parse_delattr_runtime_observation(
            observation_record,
            path=f"$.delattr_runtime_observations[{index}]",
            site_index=site_index,
        )
        for index, observation_record in enumerate(observation_records)
    )
...
def load_fixture_metaclass_behavior_runtime_observations(
    repo_root: Path | str,
    *,
    semantic_program: SemanticProgram | None = None,
) -> tuple[MetaclassBehaviorRuntimeObservation, ...]:
    """Load fixture-local metaclass behavior runtime observations."""
    root = Path(repo_root)
    observation_path = root / _METACLASS_BEHAVIOR_RUNTIME_OBSERVATION_FILENAME
    if not observation_path.is_file():
        return ()
    ...
    try:
        ...
        raise EvalOracleSchemaError(
            f"invalid metaclass behavior runtime observation JSON in "
            f"{observation_path}: {error}"
        ) from error
    _reject_generated_id_fields(raw, path="$")
    record = _expect_object(raw, path="$")
    ...
    if not observation_records:
        raise EvalOracleSchemaError(
            "metaclass behavior runtime observation file must contain at least "
            "one observation"
        )
    ...
    site_index = _metaclass_behavior_observation_site_index(program)
    return tuple(
        _parse_metaclass_behavior_runtime_observation(
            observation_record,
            path=f"$.metaclass_behavior_runtime_observations[{index}]",
            site_index=site_index,
        )
        for index, observation_record in enumerate(observation_records)
    )
...
def _eval_observation_site_index(
    program: SemanticProgram,
) -> dict[tuple[str, int, int, int, int], SourceSite]:
    """Index eligible ``eval(source)`` source sites by file/span identity."""
    ...
    for construct in program.unsupported_constructs:
        ...
        if call_site is None or not _is_eligible_eval_call_site(call_site):
            continue
        identity = _source_site_identity(construct.site)
        if identity in site_index:
            raise EvalOracleSchemaError(
                "multiple eval unsupported constructs share the same source site"
            )
        site_index[identity] = construct.site
    return site_index


def _is_eligible_eval_call_site(call_site: CallSiteFact) -> bool:
    """Return whether ``call_site`` is fixture-loadable ``eval(source)``."""
...
def _getattr_observation_site_index(
    ...
    for construct in program.unsupported_constructs:
        if construct.reason_code is not UnresolvedReasonCode.REFLECTIVE_BUILTIN:
            continue
        call_site = call_sites_by_unsupported_id.get(construct.construct_id)
        if call_site is None or not _is_eligible_getattr_call_site(call_site):
            continue
        identity = _source_site_identity(construct.site)
        if identity in site_index:
            raise EvalOracleSchemaError(
                "multiple getattr unsupported constructs share the same source site"
            )
        site_index[identity] = construct.site
    return site_index
...
def _dir_observation_site_index(
    ...
    for construct in program.unsupported_constructs:
        if construct.reason_code is not UnresolvedReasonCode.REFLECTIVE_BUILTIN:
            continue
        call_site = call_sites_by_unsupported_id.get(construct.construct_id)
        if call_site is None or not _is_eligible_dir_call_site(call_site):
            continue
        identity = _source_site_identity(construct.site)
        if identity in site_index:
            raise EvalOracleSchemaError(
                "multiple dir unsupported constructs share the same source site"
            )
        site_index[identity] = construct.site
    return site_index
...
def _vars_observation_site_index(
    ...
    for construct in program.unsupported_constructs:
        if construct.reason_code is not UnresolvedReasonCode.REFLECTIVE_BUILTIN:
            continue
        call_site = call_sites_by_unsupported_id.get(construct.construct_id)
        if call_site is None or not _is_eligible_vars_call_site(call_site):
            continue
        identity = _source_site_identity(construct.site)
        if identity in site_index:
            raise EvalOracleSchemaError(
                "multiple vars unsupported constructs share the same source site"
            )
        site_index[identity] = construct.site
    return site_index
...
def _globals_observation_site_index(
    program: SemanticProgram,
) -> dict[tuple[str, int, int, int, int], SourceSite]:
    """Index eligible ``globals()`` source sites by identity."""
    call_sites_by_unsupported_id = {
        f"unsupported:{call_site.call_site_id}": call_site
        for call_site in program.syntax.call_sites
    }
    ...
    for construct in program.unsupported_constructs:
        if construct.reason_code is not UnresolvedReasonCode.RUNTIME_MUTATION:
            continue
        call_site = call_sites_by_unsupported_id.get(construct.construct_id)
        if call_site is None or not _is_eligible_globals_call_site(call_site):
            continue
        identity = _source_site_identity(construct.site)
        if identity in site_index:
            raise EvalOracleSchemaError(
                "multiple globals unsupported constructs share the same source site"
            )
        site_index[identity] = construct.site
    return site_index


def _is_eligible_globals_call_site(call_site: CallSiteFact) -> bool:
    """Return whether ``call_site`` is a fixture-loadable ``globals()`` form."""
...
def _locals_observation_site_index(
    program: SemanticProgram,
) -> dict[tuple[str, int, int, int, int], SourceSite]:
    """Index eligible ``locals()`` source sites by identity."""
    call_sites_by_unsupported_id = {
        f"unsupported:{call_site.call_site_id}": call_site
        for call_site in program.syntax.call_sites
    }
    ...
    for construct in program.unsupported_constructs:
        if construct.reason_code is not UnresolvedReasonCode.RUNTIME_MUTATION:
            continue
        call_site = call_sites_by_unsupported_id.get(construct.construct_id)
        if call_site is None or not _is_eligible_locals_call_site(call_site):
            continue
        identity = _source_site_identity(construct.site)
        if identity in site_index:
            raise EvalOracleSchemaError(
                "multiple locals unsupported constructs share the same source site"
            )
        site_index[identity] = construct.site
    return site_index


def _delattr_observation_site_index(
    program: SemanticProgram,
) -> dict[tuple[str, int, int, int, int], SourceSite]:
    """Index eligible ``delattr(obj, name)`` source sites by identity."""
    call_sites_by_unsupported_id = {
        f"unsupported:{call_site.call_site_id}": call_site
        for call_site in program.syntax.call_sites
    }
    ...
    for construct in program.unsupported_constructs:
        if construct.reason_code is not UnresolvedReasonCode.RUNTIME_MUTATION:
            continue
        call_site = call_sites_by_unsupported_id.get(construct.construct_id)
        if call_site is None or not _is_eligible_delattr_call_site(call_site):
            continue
        identity = _source_site_identity(construct.site)
        if identity in site_index:
            raise EvalOracleSchemaError(
                "multiple delattr unsupported constructs share the same source site"
            )
        site_index[identity] = construct.site
    return site_index


def _metaclass_behavior_observation_site_index(
    program: SemanticProgram,
) -> dict[tuple[str, int, int, int, int], SourceSite]:
    """Index eligible metaclass keyword source sites by identity."""
    site_index: dict[tuple[str, int, int, int, int], SourceSite] = {}
    for construct in program.unsupported_constructs:
        ...
        if identity in site_index:
            raise EvalOracleSchemaError(
                "multiple metaclass behavior unsupported constructs share the same "
                "source site"
            )
        site_index[identity] = construct.site
    return site_index
...
def _is_eligible_locals_call_site(call_site: CallSiteFact) -> bool:
    """Return whether ``call_site`` is a fixture-loadable ``locals()`` form."""
    if call_site.argument_count != 0:
        return False
    try:
        expression = ast.parse(call_site.callee_text, mode="eval").body
...
def _setattr_observation_site_index(
    program: SemanticProgram,
) -> dict[tuple[str, int, int, int, int], SourceSite]:
    """Index eligible ``setattr(obj, name, value)`` source sites by identity."""
    call_sites_by_unsupported_id = {
        f"unsupported:{call_site.call_site_id}": call_site
        for call_site in program.syntax.call_sites
    }
    ...
    for construct in program.unsupported_constructs:
        if construct.reason_code is not UnresolvedReasonCode.RUNTIME_MUTATION:
            continue
        call_site = call_sites_by_unsupported_id.get(construct.construct_id)
        if call_site is None or not _is_eligible_setattr_call_site(call_site):
            continue
        identity = _source_site_identity(construct.site)
        if identity in site_index:
            raise EvalOracleSchemaError(
                "multiple setattr unsupported constructs share the same source site"
            )
        site_index[identity] = construct.site
    return site_index


def _is_eligible_setattr_call_site(call_site: CallSiteFact) -> bool:
    """Return whether ``call_site`` is fixture-loadable ``setattr``."""
...
def _is_eligible_delattr_call_site(call_site: CallSiteFact) -> bool:
    """Return whether ``call_site`` is fixture-loadable ``delattr(obj, name)``."""
    if call_site.argument_count != 2:
        return False
    try:
        expression = ast.parse(call_site.callee_text, mode="eval").body
...
def _parse_eval_runtime_observation(
    raw: object,
    *,
    path: str,
    site_index: dict[tuple[str, int, int, int, int], SourceSite],
) -> EvalRuntimeObservation:
    """Parse one fixture-local bounded ``eval(source)`` observation record."""
    record = _expect_object(raw, path=path)
    _validate_allowed_fields(
        record,
        _ALLOWED_EVAL_OBSERVATION_FIELDS,
        path=path,
    )
    site = _matched_eval_observation_site(
        file_path=_required_string(record, "file_path", path=path),
        start_line=_required_positive_int(record, "start_line", path=path),
        start_column=_required_non_negative_int(record, "start_column", path=path),
        end_line=_required_positive_int(record, "end_line", path=path),
...
def _parse_getattr_runtime_observation(
    raw: object,
    *,
    path: str,
    site_index: dict[tuple[str, int, int, int, int], SourceSite],
) -> GetattrRuntimeObservation:
    """Parse one fixture-local ``getattr`` runtime observation record."""
    record = _expect_object(raw, path=path)
    _validate_allowed_fields(
        record,
        _ALLOWED_GETATTR_OBSERVATION_FIELDS,
        path=path,
    )
    site = _matched_getattr_observation_site(
        file_path=_required_string(record, "file_path", path=path),
        start_line=_required_positive_int(record, "start_line", path=path),
        start_column=_required_non_negative_int(record, "start_column", path=path),
        end_line=_required_positive_int(record, "end_line", path=path),
    ...
    return GetattrRuntimeObservation(
        site=site,
        probe_identifier=_required_string(record, "probe_identifier", path=path),
        probe_contract_revision=_required_string(
            record,
...
def _parse_dir_runtime_observation(
    raw: object,
    *,
    path: str,
    site_index: dict[tuple[str, int, int, int, int], SourceSite],
    ...
    _validate_allowed_fields(
        record,
        _ALLOWED_DIR_OBSERVATION_FIELDS,
        path=path,
    )
    site = _matched_dir_observation_site(
        file_path=_required_string(record, "file_path", path=path),
        start_line=_required_positive_int(record, "start_line", path=path),
        start_column=_required_non_negative_int(record, "start_column", path=path),
        end_line=_required_positive_int(record, "end_line", path=path),
...
def _parse_vars_runtime_observation(
    raw: object,
    *,
    path: str,
    site_index: dict[tuple[str, int, int, int, int], SourceSite],
    ...
    _validate_allowed_fields(
        record,
        _ALLOWED_VARS_OBSERVATION_FIELDS,
        path=path,
    )
    site = _matched_vars_observation_site(
        file_path=_required_string(record, "file_path", path=path),
        start_line=_required_positive_int(record, "start_line", path=path),
        start_column=_required_non_negative_int(record, "start_column", path=path),
        end_line=_required_positive_int(record, "end_line", path=path),
...
def _parse_globals_runtime_observation(
    raw: object,
    *,
    path: str,
    site_index: dict[tuple[str, int, int, int, int], SourceSite],
) -> GlobalsRuntimeObservation:
    """Parse one fixture-local ``globals()`` runtime observation record."""
    record = _expect_object(raw, path=path)
    _validate_allowed_fields(
        record,
        _ALLOWED_GLOBALS_OBSERVATION_FIELDS,
        path=path,
    )
    site = _matched_globals_observation_site(
        file_path=_required_string(record, "file_path", path=path),
        start_line=_required_positive_int(record, "start_line", path=path),
        start_column=_required_non_negative_int(record, "start_column", path=path),
        end_line=_required_positive_int(record, "end_line", path=path),
    ...
    return GlobalsRuntimeObservation(
        site=site,
        probe_identifier=_required_string(record, "probe_identifier", path=path),
        probe_contract_revision=_required_string(
            record,
...
def _parse_locals_runtime_observation(
    raw: object,
    *,
    path: str,
    site_index: dict[tuple[str, int, int, int, int], SourceSite],
) -> LocalsRuntimeObservation:
    """Parse one fixture-local ``locals()`` runtime observation record."""
    record = _expect_object(raw, path=path)
    _validate_allowed_fields(
        record,
        _ALLOWED_LOCALS_OBSERVATION_FIELDS,
        path=path,
    )
    site = _matched_locals_observation_site(
        file_path=_required_string(record, "file_path", path=path),
        start_line=_required_positive_int(record, "start_line", path=path),
        start_column=_required_non_negative_int(record, "start_column", path=path),
        end_line=_required_positive_int(record, "end_line", path=path),
    ...
    return LocalsRuntimeObservation(
        site=site,
        probe_identifier=_required_string(record, "probe_identifier", path=path),
        probe_contract_revision=_required_string(
            record,
...
def _parse_setattr_runtime_observation(
    raw: object,
    *,
    path: str,
    site_index: dict[tuple[str, int, int, int, int], SourceSite],
) -> SetattrRuntimeObservation:
    """Parse one fixture-local ``setattr`` runtime observation record."""
    record = _expect_object(raw, path=path)
    _validate_allowed_fields(
        record,
        _ALLOWED_SETATTR_OBSERVATION_FIELDS,
        path=path,
    )
    site = _matched_setattr_observation_site(
        file_path=_required_string(record, "file_path", path=path),
        start_line=_required_positive_int(record, "start_line", path=path),
        start_column=_required_non_negative_int(record, "start_column", path=path),
        end_line=_required_positive_int(record, "end_line", path=path),
    ...
    return SetattrRuntimeObservation(
        site=site,
        probe_identifier=_required_string(record, "probe_identifier", path=path),
        probe_contract_revision=_required_string(
            record,
...
def _parse_delattr_runtime_observation(
    raw: object,
    *,
    path: str,
    site_index: dict[tuple[str, int, int, int, int], SourceSite],
) -> DelattrRuntimeObservation:
    """Parse one fixture-local ``delattr`` runtime observation record."""
    record = _expect_object(raw, path=path)
    _validate_allowed_fields(
        record,
        _ALLOWED_DELATTR_OBSERVATION_FIELDS,
        path=path,
    )
    site = _matched_delattr_observation_site(
        file_path=_required_string(record, "file_path", path=path),
        start_line=_required_positive_int(record, "start_line", path=path),
        start_column=_required_non_negative_int(record, "start_column", path=path),
        end_line=_required_positive_int(record, "end_line", path=path),
    ...
    return DelattrRuntimeObservation(
        site=site,
        probe_identifier=_required_string(record, "probe_identifier", path=path),
        probe_contract_revision=_required_string(
            record,
...
def _parse_metaclass_behavior_runtime_observation(
    raw: object,
    *,
    path: str,
    site_index: dict[tuple[str, int, int, int, int], SourceSite],
) -> MetaclassBehaviorRuntimeObservation:
    """Parse one fixture-local metaclass behavior observation record."""
    record = _expect_object(raw, path=path)
    _validate_allowed_fields(
        record,
        _ALLOWED_METACLASS_BEHAVIOR_OBSERVATION_FIELDS,
    ...
    site = _matched_metaclass_behavior_observation_site(
        file_path=_required_string(record, "file_path", path=path),
        start_line=_required_positive_int(record, "start_line", path=path),
        start_column=_required_non_negative_int(record, "start_column", path=path),
        end_line=_required_positive_int(record, "end_line", path=path),
    ...
    return MetaclassBehaviorRuntimeObservation(
        site=site,
        probe_identifier=_required_string(record, "probe_identifier", path=path),
        probe_contract_revision=_required_string(
            record,
...
def _matched_eval_observation_site(
    *,
    file_path: str,
    start_line: int,
    start_column: int,
    ...
    """Return the analyzed source site matching one ``eval`` observation."""
    identity = (file_path, start_line, start_column, end_line, end_column)
    site = site_index.get(identity)
    if site is None:
        raise EvalOracleSchemaError(
...
def _matched_getattr_observation_site(
    *,
    file_path: str,
    start_line: int,
    start_column: int,
    ...
    """Return the analyzed source site matching one ``getattr`` observation."""
    identity = (file_path, start_line, start_column, end_line, end_column)
    site = site_index.get(identity)
    if site is None:
        raise EvalOracleSchemaError(
            f"{path} does not match any analyzed eligible getattr source site"
        )
    if source_snippet is not None and site.snippet != source_snippet:
        raise EvalOracleSchemaError(
            f"{path}.source_snippet does not match the analyzed source site"
...
def _matched_dir_observation_site(
    *,
    file_path: str,
    start_line: int,
    start_column: int,
    ...
    if site is None:
        raise EvalOracleSchemaError(
            f"{path} does not match any analyzed eligible dir source site"
        )
    if source_snippet is not None and site.snippet != source_snippet:
        raise EvalOracleSchemaError(
            f"{path}.source_snippet does not match the analyzed source site"
...
def _matched_vars_observation_site(
    *,
    file_path: str,
    start_line: int,
    start_column: int,
    ...
    if site is None:
        raise EvalOracleSchemaError(
            f"{path} does not match any analyzed eligible vars source site"
        )
    if source_snippet is not None and site.snippet != source_snippet:
        raise EvalOracleSchemaError(
            f"{path}.source_snippet does not match the analyzed source site"
...
def _matched_globals_observation_site(
    *,
    file_path: str,
    start_line: int,
    start_column: int,
    ...
    """Return the analyzed source site matching one ``globals()`` observation."""
    identity = (file_path, start_line, start_column, end_line, end_column)
    site = site_index.get(identity)
    if site is None:
        raise EvalOracleSchemaError(
            f"{path} does not match any analyzed eligible globals source site"
        )
    if source_snippet is not None and site.snippet != source_snippet:
        raise EvalOracleSchemaError(
            f"{path}.source_snippet does not match the analyzed source site"
...
def _matched_locals_observation_site(
    *,
    file_path: str,
    start_line: int,
    start_column: int,
    ...
    """Return the analyzed source site matching one ``locals()`` observation."""
    identity = (file_path, start_line, start_column, end_line, end_column)
    site = site_index.get(identity)
    if site is None:
        raise EvalOracleSchemaError(
            f"{path} does not match any analyzed eligible locals source site"
        )
    if source_snippet is not None and site.snippet != source_snippet:
        raise EvalOracleSchemaError(
            f"{path}.source_snippet does not match the analyzed source site"
...
def _matched_setattr_observation_site(
    *,
    file_path: str,
    start_line: int,
    start_column: int,
    ...
    """Return the analyzed source site matching one ``setattr`` observation."""
    identity = (file_path, start_line, start_column, end_line, end_column)
    site = site_index.get(identity)
    if site is None:
        raise EvalOracleSchemaError(
            f"{path} does not match any analyzed eligible setattr source site"
        )
    if source_snippet is not None and site.snippet != source_snippet:
        raise EvalOracleSchemaError(
            f"{path}.source_snippet does not match the analyzed source site"
...
def _matched_delattr_observation_site(
    *,
    file_path: str,
    start_line: int,
    start_column: int,
    ...
    """Return the analyzed source site matching one ``delattr`` observation."""
    identity = (file_path, start_line, start_column, end_line, end_column)
    site = site_index.get(identity)
    if site is None:
        raise EvalOracleSchemaError(
            f"{path} does not match any analyzed eligible delattr source site"
        )
    if source_snippet is not None and site.snippet != source_snippet:
        raise EvalOracleSchemaError(
            f"{path}.source_snippet does not match the analyzed source site"
...
def _matched_metaclass_behavior_observation_site(
    *,
    file_path: str,
    start_line: int,
    start_column: int,
    ...
    """Return the analyzed source site matching one metaclass observation."""
    identity = (file_path, start_line, start_column, end_line, end_column)
    site = site_index.get(identity)
    if site is None:
        raise EvalOracleSchemaError(
            f"{path} does not match any analyzed eligible metaclass behavior "
            "source site"
        )
    if source_snippet is not None and site.snippet != source_snippet:
        raise EvalOracleSchemaError(
