"""Imported-name ``import_module(name)`` runtime-backed eval pilot tests."""
...
TASK_PATH = (
    ...
    / "evals"
    / "tasks"
    / "oracle_signal_dynamic_import_imported_name_probe.json"
)
RUN_SPEC_PATH = (
    REPO_ROOT
    / "evals"
...
QUERY = (
    "Fix unsupported dynamic import import_module(name) "
...
def test_dynamic_import_imported_name_probe_task_resolves_expected_selectors() -> None:
    """The imported-name probe resolves intended symbol and boundary selectors."""
    setup = setup_eval_oracle_task(TASK_PATH)

    assert setup.task.task_id == "oracle_signal_dynamic_import_imported_name_probe"
    assert setup.task.fixture_id == "oracle_signal_dynamic_import_imported_name_probe"
...
def test_dynamic_import_imported_name_probe_run_spec_loads_two_budget_matrix() -> None:
    """The run spec stays at 1 task x 2 budgets x 3 providers."""
    spec = eval_runs.load_eval_run_spec(RUN_SPEC_PATH)

    assert spec.plan_id == "oracle_signal_dynamic_import_imported_name_probe_matrix"
    assert len(spec.cases) == 1
    case = spec.cases[0]
    assert case.case_id == "signal_dynamic_import_imported_name_probe"
...
def test_dynamic_import_imported_name_probe_fixture_uses_only_name_shape() -> None:
    """The fixture preserves only ``from importlib import import_module`` shape."""
    ...
    aliased_import_module_imports = [
        node
        for node in module.body
        if isinstance(node, ast.ImportFrom)
        and node.module == "importlib"
        and any(alias.name == "import_module" and alias.asname for alias in node.names)
    ]
    imported_name_calls = [
        node
    ...
    forbidden_name_calls = [
        ...
        if isinstance(node, ast.Call)
        and isinstance(node.func, ast.Name)
        and node.func.id in {"__import__", "load_module"}
    ]
    ...
    assert source.count("from importlib import import_module") == 1
    assert source.count('name = "plugins.weather"') == 1
    assert source.count("import_module(name)") == 1
    ...
    assert "load_module(name)" not in source
    assert "loader.import_module(name)" not in source
    assert "globals" not in source
    assert "locals" not in source
    assert "fromlist" not in source
    assert importlib_imports == []
    assert len(import_module_imports) == 1
    assert aliased_import_module_imports == []
    assert len(imported_name_calls) == 1
    assert len(imported_name_calls[0].args) == 1
    assert imported_name_calls[0].keywords == []
    assert isinstance(imported_name_calls[0].args[0], ast.Name)
    assert imported_name_calls[0].args[0].id == "name"
    assert forbidden_name_calls == []
    assert forbidden_attribute_calls == []
    assert plugin_name.value == "plugins.weather"
    assert len(observations) == 1
    assert observations[0].site.site_id == UNSUPPORTED_SITE_ID
    assert observations[0].site.snippet == "import_module(name)"
...
def test_dynamic_import_imported_name_probe_assets_stay_internal() -> None:
    """The imported-name probe remains internal and does not widen public exports."""
    ...
    assert "oracle_signal_dynamic_import_imported_name_probe" not in context_ir.__all__
    assert not hasattr(context_ir, "oracle_signal_dynamic_import_imported_name_probe")


def test_imported_name_probe_keeps_runtime_module_out_of_static_proof() -> None:
...
def test_imported_name_probe_default_local_provider_replays_exact_fixture(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The default local-Python provider admits only this imported-name fixture."""
    ...
    result = eval_providers.build_context_ir_default_local_python_subprocess_pack(
        eval_providers.EvalProviderRequest(
            ...
            task_id="oracle_signal_dynamic_import_imported_name_probe",
            query=QUERY,
            budget=220,
        )
    )
    ...
    assert result.task_id == "oracle_signal_dynamic_import_imported_name_probe"
    ...
    assert planned_request.source_site.span.end_column == 32
    assert planned_request.source_site.snippet == "import_module(name)"
    assert planned_request.boundary_text == "import_module(name)"
    assert (
...
@pytest.mark.parametrize("budget", (100, 180, 219))
def test_imported_name_probe_default_local_provider_fails_closed_for_non_220_budget(
    monkeypatch: pytest.MonkeyPatch,
    budget: int,
) -> None:
...
def test_imported_name_probe_default_local_provider_rejects_dynamic_import_siblings(
    task_id: str,
) -> None:
...
def test_imported_name_probe_run_executes_with_additive_runtime_provenance(
    tmp_path: Path,
) -> None:
    """Context IR attaches runtime provenance while primary truth stays opaque."""
    ledger_path = tmp_path / "dynamic_import_imported_name_probe.jsonl"
    ...
    assert (
        execution.plan_id == "oracle_signal_dynamic_import_imported_name_probe_matrix"
    )
