"""Isolated zero-argument ``vars`` runtime-backed eval pilot tests."""
...
FIXTURE_ROOT = REPO_ROOT / "evals" / "fixtures" / "oracle_signal_vars_zero_probe"
TASK_PATH = REPO_ROOT / "evals" / "tasks" / "oracle_signal_vars_zero_probe.json"
RUN_SPEC_PATH = (
...
QUERY = (
    "Fix probe_local_namespace unsupported vars() returned namespace and keep digest "
    "output aligned"
)
...
def _single_provider_run_spec_path(
    ...
    *,
    ...
    """Return a temporary vars/0 probe run spec for one provider and budget."""
    spec_record = cast(
        dict[str, object],
    ...
    spec_path = tmp_path / "vars_zero_probe_single_provider.json"
...
def test_vars_zero_probe_task_resolves_expected_selectors_deterministically() -> None:
    """The isolated zero-argument probe resolves the intended selectors."""
    setup = setup_eval_oracle_task(TASK_PATH)

    assert setup.task.task_id == "oracle_signal_vars_zero_probe"
    assert setup.task.fixture_id == "oracle_signal_vars_zero_probe"
...
def test_vars_zero_probe_run_spec_loads_cleanly_through_runner() -> None:
    """The isolated zero-argument probe run spec stays valid runner input."""
    spec = eval_runs.load_eval_run_spec(RUN_SPEC_PATH)

    assert spec.plan_id == "oracle_signal_vars_zero_probe_matrix"
    assert len(spec.cases) == 1
    case = spec.cases[0]
    assert case.case_id == "signal_vars_zero_probe"
    assert case.task_path == "evals/tasks/oracle_signal_vars_zero_probe.json"
    assert case.query == QUERY
    assert case.budgets == PROBE_BUDGETS
    assert case.providers == PROBE_PROVIDERS


def test_vars_zero_probe_fixture_uses_zero_argument_namespace_payload() -> None:
    """The fixture preserves the eval-only zero-argument ``vars()`` branch."""
    source = (FIXTURE_ROOT / "main.py").read_text(encoding="utf-8")
    observations = load_fixture_vars_runtime_observations(FIXTURE_ROOT)

    assert source.count("vars()") == 1
    assert "vars(obj)" not in source
    assert len(observations) == 1
    assert observations[0].site.snippet == "vars()"
...
def test_vars_zero_probe_assets_stay_internal() -> None:
    ...
    assert "oracle_signal_vars_zero_probe" not in context_ir.__all__
    assert not hasattr(context_ir, "oracle_signal_vars_zero_probe")


def test_vars_zero_probe_default_subprocess_provider_owns_runtime_provenance(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The exact provider replays vars() through subprocess-owned provenance."""
    setup = setup_eval_oracle_task(TASK_PATH)
    ...
    provider_result = (
        eval_providers.build_context_ir_default_local_python_subprocess_pack(
            eval_providers.EvalProviderRequest(
                ...
                task_id="oracle_signal_vars_zero_probe",
                query=QUERY,
                budget=100,
            )
        )
    ...
    assert planned_request.form_label == "reflective_builtin:vars/0"
    assert planned_request.boundary_text == "vars()"
...
def test_vars_zero_probe_default_subprocess_provider_fails_closed_for_other_tasks() -> (
    None
):
...
def test_vars_zero_probe_default_subprocess_provider_runs_via_run_spec_name(
    tmp_path: Path,
) -> None:
    ...
    ledger_path = tmp_path / "vars_zero_probe_default_subprocess.jsonl"
...
def test_vars_zero_probe_run_executes_with_additive_runtime_provenance(
    tmp_path: Path,
) -> None:
    """Run execution preserves unsupported primary truth plus runtime support."""
    ledger_path = tmp_path / "vars_zero_probe.jsonl"
    ...
    records = _parsed_ledger_records(ledger_path)
    assert execution.plan_id == "oracle_signal_vars_zero_probe_matrix"
...
def test_vars_zero_probe_summary_surfaces_internal_capability_accounting(
    tmp_path: Path,
) -> None:
    """The zero-argument pilot renders accounting without widening claims."""
    ledger_path = tmp_path / "vars_zero_probe.jsonl"
