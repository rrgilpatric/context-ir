"""Isolated bounded ``eval(source)`` runtime-backed eval pilot tests."""
...
FIXTURE_ROOT = REPO_ROOT / "evals" / "fixtures" / "oracle_signal_eval_probe"
TASK_PATH = REPO_ROOT / "evals" / "tasks" / "oracle_signal_eval_probe.json"
RUN_SPEC_PATH = (
    REPO_ROOT / "evals" / "run_specs" / "oracle_signal_eval_probe_matrix.json"
)
...
QUERY = "eval(source) returned_value"
UNSUPPORTED_UNIT_ID = "unsupported:call:main.py:3:11"
EVALUATED_SOURCE = '"eval-probe-value"'
SOURCE_SHA256 = hashlib.sha256(EVALUATED_SOURCE.encode("utf-8")).hexdigest()
RUNTIME_PAYLOAD = (
    ("evaluation_outcome", "returned_value"),
    ("result_type", "builtins.str"),
)
EXPECTED_CONTEXT_IR_SELECTED_UNIT_IDS = (
    "def:main.py:main.probe_eval_source",
    "def:main.py:main.render_probe_digest",
    UNSUPPORTED_UNIT_ID,
)
EXPECTED_CONTEXT_IR_SELECTED_UNIT_IDS_BY_BUDGET = {
    220: EXPECTED_CONTEXT_IR_SELECTED_UNIT_IDS,
    100: EXPECTED_CONTEXT_IR_SELECTED_UNIT_IDS,
}
...
def _single_provider_run_spec_path(
    ...
    *,
    ...
    """Return a temporary eval-probe run spec for one provider and budget."""
    spec_record = cast(
        dict[str, object],
    ...
    spec_path = tmp_path / "eval_probe_single_provider.json"
...
def test_eval_probe_task_resolves_expected_selectors_deterministically() -> None:
    """The isolated eval(source) probe resolves the intended selectors."""
    setup = setup_eval_oracle_task(TASK_PATH)

    assert setup.task.task_id == "oracle_signal_eval_probe"
    assert setup.task.fixture_id == "oracle_signal_eval_probe"
...
def test_eval_probe_run_spec_loads_budget_matrix() -> None:
    """The eval(source) probe run spec is one task x two budgets x three providers."""
    spec = eval_runs.load_eval_run_spec(RUN_SPEC_PATH)

    assert spec.plan_id == "oracle_signal_eval_probe_matrix"
    assert len(spec.cases) == 1
    case = spec.cases[0]
    assert case.case_id == "signal_eval_probe"
    assert case.task_path == "evals/tasks/oracle_signal_eval_probe.json"
    assert case.query == QUERY
    assert case.budgets == PROBE_BUDGETS
    assert case.providers == PROBE_PROVIDERS


def test_eval_probe_fixture_uses_bounded_literal_source_shape() -> None:
    """The fixture preserves the eval-only bounded ``eval(source)`` branch."""
    source = (FIXTURE_ROOT / "main.py").read_text(encoding="utf-8")
    module = ast.parse(source)
    observations = load_fixture_eval_runtime_observations(FIXTURE_ROOT)
    ...
    probe_function = next(
        ...
        if isinstance(node, ast.FunctionDef) and node.name == "probe_eval_source"
    )
    ...
    evaluated_source = cast(ast.Constant, source_assignment.value)
    evaluated_tree = ast.parse(cast(str, evaluated_source.value), mode="eval")

    assert source.count("eval(source)") == 1
    assert "eval(source," not in source
    assert "eval(source=" not in source
    assert "exec(" not in source
    assert len(eval_calls) == 1
    assert exec_calls == []
    assert len(eval_calls[0].args) == 1
    assert eval_calls[0].keywords == []
    assert isinstance(eval_calls[0].args[0], ast.Name)
    assert eval_calls[0].args[0].id == "source"
    assert isinstance(evaluated_source.value, str)
    assert evaluated_source.value == EVALUATED_SOURCE
    assert isinstance(evaluated_tree.body, ast.Constant)
    assert isinstance(evaluated_tree.body.value, str)
    assert not any(
        isinstance(
            node,
            (
        ...
        for node in ast.walk(evaluated_tree)
    )
    assert len(observations) == 1
    assert observations[0].site.snippet == "eval(source)"
...
def test_eval_probe_assets_stay_internal() -> None:
    """The isolated eval(source) probe does not widen public exports."""
    ...
    assert "oracle_signal_eval_probe" not in context_ir.__all__
    assert not hasattr(context_ir, "oracle_signal_eval_probe")


def test_eval_probe_default_subprocess_provider_owns_runtime_provenance(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The exact provider replays eval(source) through subprocess provenance."""
    setup = setup_eval_oracle_task(TASK_PATH)
    ...
    provider_result = (
        eval_providers.build_context_ir_default_local_python_subprocess_pack(
            eval_providers.EvalProviderRequest(
                ...
                task_id="oracle_signal_eval_probe",
                query=QUERY,
                budget=100,
            )
        )
    ...
    assert planned_request.form_label == "exec_or_eval:eval/1"
    assert planned_request.boundary_text == "eval(source)"
...
def test_eval_probe_default_subprocess_provider_runs_via_run_spec_name(
    tmp_path: Path,
) -> None:
    """Run-spec provider dispatch preserves provider-owned eval provenance."""
    spec_path = _single_provider_run_spec_path(
        tmp_path,
        provider_name=DEFAULT_LOCAL_PYTHON_SUBPROCESS_PROVIDER,
        budget=100,
    )
    ledger_path = tmp_path / "eval_probe_default_subprocess.jsonl"
...
def test_eval_probe_run_executes_with_additive_runtime_provenance(
    tmp_path: Path,
) -> None:
    """Run execution preserves unsupported primary truth plus runtime support."""
    ledger_path = tmp_path / "eval_probe.jsonl"
    ...
    records = _parsed_ledger_records(ledger_path)
    assert execution.plan_id == "oracle_signal_eval_probe_matrix"
    ...
    for budget in PROBE_BUDGETS:
        ...
        assert (
            tuple(selected_unit_ids)
            == EXPECTED_CONTEXT_IR_SELECTED_UNIT_IDS_BY_BUDGET[budget]
        )
        assert metrics["uncertainty_honesty"] == 1.0
