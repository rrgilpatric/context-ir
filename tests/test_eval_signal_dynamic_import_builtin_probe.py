"""Builtin ``__import__(name)`` runtime-backed eval pilot tests."""
...
FIXTURE_ROOT = (
    REPO_ROOT / "evals" / "fixtures" / "oracle_signal_dynamic_import_builtin_probe"
)
TASK_PATH = (
    REPO_ROOT / "evals" / "tasks" / "oracle_signal_dynamic_import_builtin_probe.json"
)
RUN_SPEC_PATH = (
    REPO_ROOT
    / "evals"
...
def test_dynamic_import_builtin_probe_task_resolves_expected_selectors() -> None:
    """The builtin probe resolves the intended symbol and boundary selectors."""
    setup = setup_eval_oracle_task(TASK_PATH)

    assert setup.task.task_id == "oracle_signal_dynamic_import_builtin_probe"
    assert setup.task.fixture_id == "oracle_signal_dynamic_import_builtin_probe"
...
def test_dynamic_import_builtin_probe_run_spec_loads_two_budget_matrix() -> None:
    """The run spec stays at 1 task x 2 budgets x 3 providers."""
    spec = eval_runs.load_eval_run_spec(RUN_SPEC_PATH)

    assert spec.plan_id == "oracle_signal_dynamic_import_builtin_probe_matrix"
    assert len(spec.cases) == 1
    case = spec.cases[0]
    assert case.case_id == "signal_dynamic_import_builtin_probe"
...
def test_dynamic_import_builtin_probe_fixture_uses_only_builtin_shape() -> None:
    """The fixture preserves the builtin ``__import__(name)`` case."""
    ...
    forbidden_name_calls = [
        ...
        if isinstance(node, ast.Call)
        and isinstance(node.func, ast.Name)
        and node.func.id in {"import_module", "load_module"}
    ]
    ...
    assert source.count("__import__(name)") == 1
    ...
    assert len(builtin_import_calls) == 1
    assert len(builtin_import_calls[0].args) == 1
    assert builtin_import_calls[0].keywords == []
    assert isinstance(builtin_import_calls[0].args[0], ast.Name)
    assert builtin_import_calls[0].args[0].id == "name"
    ...
    assert observations[0].site.snippet == "__import__(name)"
...
def test_dynamic_import_builtin_probe_assets_stay_internal() -> None:
    """The builtin probe remains internal and does not widen public exports."""
    ...
    assert "oracle_signal_dynamic_import_builtin_probe" not in context_ir.__all__
    assert not hasattr(context_ir, "oracle_signal_dynamic_import_builtin_probe")


def test_dynamic_import_builtin_probe_keeps_runtime_module_out_of_static_proof() -> (
    None
):
...
def test_dynamic_import_builtin_probe_run_executes_with_additive_runtime_provenance(
    tmp_path: Path,
) -> None:
    """Context IR attaches runtime provenance while primary truth stays opaque."""
    ledger_path = tmp_path / "dynamic_import_builtin_probe.jsonl"
    ...
    assert execution.plan_id == "oracle_signal_dynamic_import_builtin_probe_matrix"
...
def test_dynamic_import_builtin_probe_default_local_provider_replays_exact_fixture(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The default local-Python provider admits only this builtin fixture."""
    ...
    result = eval_providers.build_context_ir_default_local_python_subprocess_pack(
        eval_providers.EvalProviderRequest(
            ...
            task_id="oracle_signal_dynamic_import_builtin_probe",
            query=QUERY,
            budget=220,
        )
    )
    ...
    assert result.task_id == "oracle_signal_dynamic_import_builtin_probe"
    ...
    assert planned_request.source_site.span.end_column == 20
    assert planned_request.source_site.snippet == "__import__(name)"
    assert planned_request.boundary_text == "__import__(name)"
    assert (
        planned_request.family_label is eval_providers.RuntimeProbeFamily.DYNAMIC_IMPORT
    )
    assert planned_request.form_label == "dynamic_import:__import__/1"
...
def test_builtin_probe_default_local_provider_fails_closed_for_non_220_budget(
    monkeypatch: pytest.MonkeyPatch,
    budget: int,
) -> None:
