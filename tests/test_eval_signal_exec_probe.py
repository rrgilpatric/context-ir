...
def _single_provider_run_spec_path(
    ...
    *,
    ...
    """Return a temporary exec-probe run spec for one provider and budget."""
    spec_record = cast(
        dict[str, object],
    ...
    spec_path = tmp_path / "exec_probe_single_provider.json"
...
def test_exec_probe_run_spec_loads_budget_matrix() -> None:
    """The exec(source) probe run spec is one task x two budgets x three providers."""
...
def test_exec_probe_default_subprocess_provider_owns_runtime_provenance(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The exact provider replays exec(source) through subprocess provenance."""
    setup = setup_eval_oracle_task(TASK_PATH)
    ...
    provider_result = (
        eval_providers.build_context_ir_default_local_python_subprocess_pack(
            eval_providers.EvalProviderRequest(
                ...
                task_id="oracle_signal_exec_probe",
                query=QUERY,
                budget=100,
            )
        )
    ...
    assert planned_request.form_label == "exec_or_eval:exec/1"
    assert planned_request.boundary_text == "exec(source)"
...
def test_exec_probe_default_subprocess_provider_runs_via_run_spec_name(
    tmp_path: Path,
) -> None:
    """Run-spec provider dispatch preserves provider-owned exec provenance."""
    spec_path = _single_provider_run_spec_path(
        tmp_path,
        provider_name=DEFAULT_LOCAL_PYTHON_SUBPROCESS_PROVIDER,
        budget=100,
    )
    ledger_path = tmp_path / "exec_probe_default_subprocess.jsonl"
