...
def _write_local_python_hasattr_literal_probe_program(tmp_path: Path) -> None:
    """Write the exact direct-literal hasattr replay-input source."""
    (tmp_path / "main.py").write_text(
        textwrap.dedent(
            """
...
def _write_local_python_getattr_literal_probe_program(tmp_path: Path) -> None:
    """Write the exact direct-literal getattr replay-input source."""
    (tmp_path / "main.py").write_text(
        textwrap.dedent(
            """
...
def _write_local_python_setattr_literal_probe_program(tmp_path: Path) -> None:
    """Write the exact direct-literal setattr replay-input source."""
    (tmp_path / "main.py").write_text(
        textwrap.dedent(
            """
            class ProbeTarget:
...
def _default_local_python_subprocess_hasattr_literal_facade_fixture(
    tmp_path: Path,
) -> tuple[
    SemanticContextResponse,
    SemanticMissEvidence,
    ...
    """Build a prior facade response with the exact literal-hasattr request."""
    _write_local_python_hasattr_literal_probe_program(tmp_path)
    previous_response = compile_repository_context(
        SemanticContextRequest(
            repo_root=tmp_path,
            query="literal hasattr probe",
            budget=32,
        )
    )
    ...
    assert request.form_label == "reflective_builtin:hasattr/2"
    assert request.replay_target_seed == "main.probe_literal_attribute"
    assert all(
...
def _default_local_python_subprocess_getattr_literal_facade_fixture(
    tmp_path: Path,
) -> tuple[
    SemanticContextResponse,
    SemanticMissEvidence,
    ...
    """Build a prior facade response with the exact literal-getattr request."""
    _write_local_python_getattr_literal_probe_program(tmp_path)
    previous_response = compile_repository_context(
        SemanticContextRequest(
            repo_root=tmp_path,
            query="literal getattr probe",
            budget=32,
        )
    )
    ...
    assert request.form_label == "reflective_builtin:getattr/2"
    assert request.replay_target_seed == "main.probe_literal_attribute"
    assert all(
        "bit_length" not in (symbol_id, symbol.qualified_name)
        for symbol_id, symbol in previous_response.program.resolved_symbols.items()
...
def _default_local_python_subprocess_setattr_literal_facade_fixture(
    tmp_path: Path,
) -> tuple[
    SemanticContextResponse,
    SemanticMissEvidence,
    ...
    """Build a prior facade response with the exact literal-setattr request."""
    _write_local_python_setattr_literal_probe_program(tmp_path)
    previous_response = compile_repository_context(
        SemanticContextRequest(
            repo_root=tmp_path,
            query="literal setattr probe",
            budget=32,
        )
    )
    ...
    assert request.form_label == "runtime_mutation:setattr/3"
    assert request.replay_target_seed == "main.probe_set_literal_attribute"
    assert all(
        "flag" not in (symbol_id, symbol.qualified_name)
        for symbol_id, symbol in previous_response.program.resolved_symbols.items()
    )
...
def test_default_local_python_subprocess_recompile_facade_runs_literal_hasattr(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The facade carries direct-literal hasattr replay inputs by default."""
    (
        previous_response,
        miss_evidence,
        diagnostic,
        ...
        request,
        unsupported_id,
    ) = _default_local_python_subprocess_hasattr_literal_facade_fixture(tmp_path)
    runtime_assumptions = _runner_runtime_assumptions()
    runner_environment = _local_python_runner_environment(tmp_path)
    runner_assumptions = _runner_assumptions()
    original_run = runtime_probe_execution.subprocess.run
...
def test_default_local_python_subprocess_recompile_facade_runs_literal_getattr(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The facade carries direct-literal getattr replay inputs by default."""
    (
        previous_response,
        miss_evidence,
        diagnostic,
        ...
        request,
        unsupported_id,
    ) = _default_local_python_subprocess_getattr_literal_facade_fixture(tmp_path)
    runtime_assumptions = _runner_runtime_assumptions()
    runner_environment = _local_python_runner_environment(tmp_path)
    runner_assumptions = _runner_assumptions()
    original_run = runtime_probe_execution.subprocess.run
...
def test_default_local_python_subprocess_recompile_facade_runs_literal_setattr(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The facade carries direct-literal setattr replay inputs by default."""
    (
        previous_response,
        miss_evidence,
        diagnostic,
        ...
        request,
        unsupported_id,
    ) = _default_local_python_subprocess_setattr_literal_facade_fixture(tmp_path)
    runtime_assumptions = _runner_runtime_assumptions()
    runner_environment = _local_python_runner_environment(tmp_path)
    runner_assumptions = _runner_assumptions()
    original_run = runtime_probe_execution.subprocess.run
    ...
    expected_replay_inputs = (
        _probe_field("object_type", "main.ProbeTarget"),
        _probe_field("attribute_name", "flag"),
        _probe_field("assigned_value_type", "builtins.str"),
        _probe_field("assigned_value_literal", "ready"),
    )
    ...
    assert preparation.request_plan is plan
    assert runner_request.request is request
    assert runner_request.replay_artifact.replay_inputs[-4:] == expected_replay_inputs
    assert attempt.request is request
    assert attempt.outcome is runtime_probe_results.RuntimeProbeResultOutcome.OBSERVED
    assert attempt.normalized_payload == expected_payload
    assert attempt.observed_replay_inputs == ()
    assert attempt.durable_artifact_reference == expected_artifact_reference
    assert isinstance(observed_result, runtime_probe_results.RuntimeProbeObservedResult)
    assert observed_result.request is request
    assert observed_result.normalized_payload == expected_payload
    ...
    assert tuple(
        (field.key, field.value) for field in admission.observation.replay_inputs[-4:]
    ) == (
