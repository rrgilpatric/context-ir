"""Builtins-attribute dynamic import runtime-backed eval pilot tests."""
...
TASK_PATH = (
    ...
    / "evals"
    / "tasks"
    / "oracle_signal_dynamic_import_builtins_attr_probe.json"
)
RUN_SPEC_PATH = (
    REPO_ROOT
    / "evals"
...
QUERY = (
    "Fix unsupported dynamic import builtins.__import__(name) "
...
def test_dynamic_import_builtins_attr_probe_task_resolves_selectors() -> None:
    """The builtins-attribute probe resolves symbol and boundary selectors."""
    setup = setup_eval_oracle_task(TASK_PATH)

    assert setup.task.task_id == "oracle_signal_dynamic_import_builtins_attr_probe"
    assert setup.task.fixture_id == "oracle_signal_dynamic_import_builtins_attr_probe"
...
def test_dynamic_import_builtins_attr_probe_run_spec_loads_matrix() -> None:
    """The run spec stays at 1 task x 2 budgets x 3 providers."""
    spec = eval_runs.load_eval_run_spec(RUN_SPEC_PATH)

    assert spec.plan_id == "oracle_signal_dynamic_import_builtins_attr_probe_matrix"
    assert len(spec.cases) == 1
    case = spec.cases[0]
    assert case.case_id == "signal_dynamic_import_builtins_attr_probe"
...
def test_dynamic_import_builtins_attr_probe_fixture_uses_only_exact_shape() -> None:
    """The fixture preserves only exact ``import builtins`` attribute-call form."""
    ...
    aliased_builtins_imports = [
        node
        for node in module.body
        if isinstance(node, ast.Import)
        and any(alias.name == "builtins" and alias.asname for alias in node.names)
    ]
    importlib_imports = [
        node
        for node in module.body
    ...
    assert source.count("import builtins") == 1
    assert source.count("import sys") == 1
    assert source.count('name = "plugins.weather"') == 1
    assert source.count("builtins.__import__(name)") == 1
    assert source.count("sys.modules[name]") == 1
    assert "import builtins as" not in source
    assert "from builtins import __import__" not in source
    assert "importlib" not in source
    assert "__import__(name," not in source
    assert '__import__("plugins.weather")' not in source
    assert "fromlist" not in source
    assert "globals" not in source
    assert "locals" not in source
    assert len(builtins_imports) == 1
    assert aliased_builtins_imports == []
    assert importlib_imports == []
    assert from_imports == []
    assert len(builtins_import_calls) == 1
    assert len(builtins_import_calls[0].args) == 1
    assert builtins_import_calls[0].keywords == []
    assert isinstance(builtins_import_calls[0].args[0], ast.Name)
    assert builtins_import_calls[0].args[0].id == "name"
    ...
    assert observations[0].site.snippet == "builtins.__import__(name)"
    ...
    assert observations[0].site.span.end_column == 29
    assert tuple(
...
def test_dynamic_import_builtins_attr_probe_assets_stay_internal() -> None:
    """The builtins-attribute probe remains internal and adds no public exports."""
    ...
    assert "oracle_signal_dynamic_import_builtins_attr_probe" not in context_ir.__all__
    assert not hasattr(context_ir, "oracle_signal_dynamic_import_builtins_attr_probe")


def test_dynamic_import_builtins_attr_probe_keeps_runtime_module_static_free() -> None:
...
def test_dynamic_import_builtins_attr_probe_run_preserves_runtime_provenance(
    tmp_path: Path,
) -> None:
    """Context IR attaches additive runtime provenance while truth stays opaque."""
    ledger_path = tmp_path / "dynamic_import_builtins_attr_probe.jsonl"
...
def test_builtins_attr_probe_default_local_provider_replays_exact_fixture(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The default local-Python provider admits only this builtins-attribute fixture."""
    ...
    result = eval_providers.build_context_ir_default_local_python_subprocess_pack(
        eval_providers.EvalProviderRequest(
            ...
            task_id="oracle_signal_dynamic_import_builtins_attr_probe",
            query=QUERY,
            budget=220,
        )
    )
    ...
    assert result.task_id == "oracle_signal_dynamic_import_builtins_attr_probe"
    ...
    assert planned_request.source_site.span.end_column == 29
    assert planned_request.source_site.snippet == "builtins.__import__(name)"
    assert planned_request.boundary_text == "builtins.__import__(name)"
    assert (
        planned_request.family_label is eval_providers.RuntimeProbeFamily.DYNAMIC_IMPORT
    )
    assert planned_request.form_label == "dynamic_import:builtins.__import__/1"
...
def test_builtins_attr_probe_default_local_provider_fails_closed_for_non_220_budget(
    monkeypatch: pytest.MonkeyPatch,
    budget: int,
) -> None:
