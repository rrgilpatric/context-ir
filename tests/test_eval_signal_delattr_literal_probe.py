"""Direct-literal ``delattr`` runtime-backed eval pilot tests."""
...
FIXTURE_ROOT = REPO_ROOT / "evals" / "fixtures" / "oracle_signal_delattr_literal_probe"
TASK_PATH = REPO_ROOT / "evals" / "tasks" / "oracle_signal_delattr_literal_probe.json"
RUN_SPEC_PATH = (
    REPO_ROOT
    / "evals"
    / "run_specs"
...
QUERY = (
    'Fix probe_delete_literal_attribute unsupported delattr(obj, "flag") '
    "deleted attribute and keep digest output aligned"
)
...
def _load_fixture_module() -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location(
        "oracle_signal_delattr_literal_probe_fixture",
...
def test_delattr_literal_probe_fixture_uses_only_direct_literal_shape() -> None:
    """The fixture preserves exactly ``delattr(obj, "flag")``."""
    source = (FIXTURE_ROOT / "main.py").read_text(encoding="utf-8")
    module = ast.parse(source)
    observations = load_fixture_delattr_runtime_observations(FIXTURE_ROOT)
    ...
    forbidden_calls = [
        ...
        if isinstance(node, ast.Call)
        and isinstance(node.func, ast.Name)
        and node.func.id in {"getattr", "hasattr", "setattr", "vars", "dir"}
    ]
    name_assignments = [
        node
        for node in ast.walk(module)
    ...
    call = delattr_calls[0]

    assert source.count('delattr(obj, "flag")') == 1
    assert "delattr(obj, name)" not in source
    assert "getattr(" not in source
    assert "hasattr(" not in source
    assert "setattr(" not in source
    assert "vars(" not in source
    assert "dir(" not in source
    assert len(delattr_calls) == 1
    ...
    assert observations[0].site.snippet == 'delattr(obj, "flag")'
    ...
    assert observations[0].site.span.end_column == 24
    assert tuple(
...
def test_delattr_literal_probe_resolves_without_static_attribute_proof() -> None:
    ...
    assert setup.task.task_id == "oracle_signal_delattr_literal_probe"
    assert setup.task.fixture_id == "oracle_signal_delattr_literal_probe"
    ...
    assert unsupported_construct.site.snippet == 'delattr(obj, "flag")'
...
def test_delattr_literal_probe_default_local_provider_replays_exact_fixture(
    ...
    result = eval_providers.build_context_ir_default_local_python_subprocess_pack(
        eval_providers.EvalProviderRequest(
            ...
            task_id="oracle_signal_delattr_literal_probe",
            query=QUERY,
            budget=220,
        )
    )
    ...
    assert result.task_id == "oracle_signal_delattr_literal_probe"
    ...
    assert planned_request.boundary_text == 'delattr(obj, "flag")'
    assert (
        planned_request.family_label
        is eval_providers.RuntimeProbeFamily.RUNTIME_MUTATION
    )
    assert planned_request.form_label == "runtime_mutation:delattr/2"
    assert planned_request.replay_target_seed == "main.probe_delete_literal_attribute"
...
def test_delattr_literal_probe_default_local_provider_rejects_wrong_plan_fields() -> (
    None
):
    ...
    object.__setattr__(plan.requests[0], "boundary_text", "delattr(obj, name)")
...
def test_delattr_literal_probe_default_local_provider_rejects_literal_siblings(
    task_id: str,
) -> None:
...
def test_delattr_literal_probe_run_spec_loads_two_budget_matrix() -> None:
    """The run spec stays at 1 task x 2 budgets x 3 providers."""
    spec = eval_runs.load_eval_run_spec(RUN_SPEC_PATH)

    assert spec.plan_id == "oracle_signal_delattr_literal_probe_matrix"
    assert len(spec.cases) == 1
    case = spec.cases[0]
    assert case.case_id == "signal_delattr_literal_probe"
    assert case.task_path == "evals/tasks/oracle_signal_delattr_literal_probe.json"
    assert case.query == QUERY
    assert case.budgets == PROBE_BUDGETS
    assert case.providers == PROBE_PROVIDERS


def test_delattr_literal_probe_digest_is_deterministic() -> None:
    """The direct-literal fixture executes the deletion branch."""
    module = _load_fixture_module()
    render_probe_digest = cast(Callable[[], str], module.render_probe_digest)

    assert render_probe_digest() == "delattr_literal:deleted"


def test_delattr_literal_probe_assets_stay_internal() -> None:
    ...
    assert "oracle_signal_delattr_literal_probe" not in context_ir.__all__
    assert not hasattr(context_ir, "oracle_signal_delattr_literal_probe")


def test_delattr_literal_probe_run_preserves_additive_runtime_boundary(
    tmp_path: Path,
) -> None:
    """The matrix locks selected units, baselines, and additive provenance."""
    ledger_path = tmp_path / "delattr_literal_probe.jsonl"
    ...
    assert execution.plan_id == "oracle_signal_delattr_literal_probe_matrix"
