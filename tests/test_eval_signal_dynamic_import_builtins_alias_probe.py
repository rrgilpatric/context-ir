"""Builtins-alias dynamic import runtime-backed eval pilot tests."""
...
TASK_PATH = (
    ...
    / "evals"
    / "tasks"
    / "oracle_signal_dynamic_import_builtins_alias_probe.json"
)
RUN_SPEC_PATH = (
    REPO_ROOT
    / "evals"
...
QUERY = (
    "Fix unsupported dynamic import loader.__import__(name) "
...
def test_dynamic_import_builtins_alias_probe_task_resolves_selectors() -> None:
    """The builtins-alias probe resolves symbol and boundary selectors."""
    setup = setup_eval_oracle_task(TASK_PATH)

    assert setup.task.task_id == "oracle_signal_dynamic_import_builtins_alias_probe"
    assert setup.task.fixture_id == "oracle_signal_dynamic_import_builtins_alias_probe"
...
def test_dynamic_import_builtins_alias_probe_run_spec_loads_matrix() -> None:
    """The run spec stays at 1 task x 2 budgets x 3 providers."""
    spec = eval_runs.load_eval_run_spec(RUN_SPEC_PATH)

    assert spec.plan_id == "oracle_signal_dynamic_import_builtins_alias_probe_matrix"
    assert len(spec.cases) == 1
    case = spec.cases[0]
    assert case.case_id == "signal_dynamic_import_builtins_alias_probe"
...
def test_dynamic_import_builtins_alias_probe_fixture_uses_only_exact_shape() -> None:
    """The fixture preserves only exact ``import builtins as loader`` form."""
    ...
    loader_imports = [
        ...
        and any(
            alias.name == "builtins" and alias.asname == "loader"
            for alias in node.names
        )
    ]
    plain_builtins_imports = [
        node
        for node in module.body
        if isinstance(node, ast.Import)
        and any(
            alias.name == "builtins" and alias.asname is None for alias in node.names
        )
    ]
    other_builtins_alias_imports = [
        node
        for node in module.body
        if isinstance(node, ast.Import)
        and any(
    ...
    assert source.count("import builtins as loader") == 1
    assert source.count("import sys") == 1
    assert source.count('name = "plugins.weather"') == 1
    assert source.count("loader.__import__(name)") == 1
    assert source.count("sys.modules[name]") == 1
    assert "import builtins\n" not in source
    assert "builtins.__import__(name)" not in source
    assert "from builtins import __import__" not in source
    assert "importlib" not in source
    assert "__import__(name," not in source
    assert 'loader.__import__("plugins.weather")' not in source
    assert "fromlist" not in source
    assert "globals" not in source
    assert "locals" not in source
    assert len(loader_imports) == 1
    assert plain_builtins_imports == []
    assert other_builtins_alias_imports == []
    assert importlib_imports == []
    assert from_imports == []
    assert len(loader_import_calls) == 1
    assert len(loader_import_calls[0].args) == 1
    assert loader_import_calls[0].keywords == []
    assert isinstance(loader_import_calls[0].args[0], ast.Name)
    assert loader_import_calls[0].args[0].id == "name"
    assert forbidden_builtins_attr_calls == []
    assert forbidden_import_module_attrs == []
    ...
    assert observations[0].site.snippet == "loader.__import__(name)"
    ...
    assert observations[0].site.span.end_column == 27
    assert tuple(
...
def test_dynamic_import_builtins_alias_probe_assets_stay_internal() -> None:
    """The builtins-alias probe remains internal and adds no public exports."""
    ...
    assert "oracle_signal_dynamic_import_builtins_alias_probe" not in context_ir.__all__
    assert not hasattr(context_ir, "oracle_signal_dynamic_import_builtins_alias_probe")


def test_dynamic_import_builtins_alias_probe_keeps_runtime_module_static_free() -> None:
    """Runtime evidence does not add static edges or selected plugin symbols."""
    setup = setup_eval_oracle_task(TASK_PATH)
...
def test_dynamic_import_builtins_alias_probe_run_preserves_runtime_provenance(
    tmp_path: Path,
) -> None:
    """Context IR attaches additive runtime provenance while truth stays opaque."""
    ledger_path = tmp_path / "dynamic_import_builtins_alias_probe.jsonl"
...
def test_builtins_alias_probe_default_local_provider_replays_exact_fixture(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The default local-Python provider admits only this builtins-alias fixture."""
    ...
    result = eval_providers.build_context_ir_default_local_python_subprocess_pack(
        eval_providers.EvalProviderRequest(
            ...
            task_id="oracle_signal_dynamic_import_builtins_alias_probe",
            query=QUERY,
            budget=220,
        )
    )
    ...
    assert result.task_id == "oracle_signal_dynamic_import_builtins_alias_probe"
    ...
    assert planned_request.source_site.span.start_line == 7
    assert planned_request.source_site.span.start_column == 4
    assert planned_request.source_site.span.end_line == 7
    assert planned_request.source_site.span.end_column == 27
    assert planned_request.source_site.snippet == "loader.__import__(name)"
    assert planned_request.boundary_text == "loader.__import__(name)"
    assert (
        planned_request.family_label is eval_providers.RuntimeProbeFamily.DYNAMIC_IMPORT
    )
    assert planned_request.form_label == "dynamic_import:loader.__import__/1"
...
def test_builtins_alias_probe_default_local_provider_fails_closed_for_non_220_budget(
    monkeypatch: pytest.MonkeyPatch,
    budget: int,
) -> None:
