"""Isolated defaulted ``getattr`` runtime-backed eval pilot tests."""
...
FIXTURE_ROOT = REPO_ROOT / "evals" / "fixtures" / "oracle_signal_getattr_default_probe"
TASK_PATH = REPO_ROOT / "evals" / "tasks" / "oracle_signal_getattr_default_probe.json"
RUN_SPEC_PATH = (
    REPO_ROOT
    / "evals"
    / "run_specs"
...
RUNTIME_PAYLOAD = (("lookup_outcome", "returned_default_value"),)
...
def _single_provider_run_spec_path(
    ...
    *,
    ...
    """Return a temporary defaulted getattr run spec for one provider."""
    spec_record = cast(
        dict[str, object],
    ...
    spec_path = tmp_path / "getattr_default_probe_single_provider.json"
...
def test_getattr_default_probe_task_resolves_expected_selectors_deterministically() -> (
    None
):
    """The isolated probe resolves the intended symbol and unsupported selectors."""
    setup = setup_eval_oracle_task(TASK_PATH)

    assert setup.task.task_id == "oracle_signal_getattr_default_probe"
    assert setup.task.fixture_id == "oracle_signal_getattr_default_probe"
...
def test_getattr_default_probe_run_spec_loads_cleanly_through_runner() -> None:
    """The isolated probe run spec stays valid runner input."""
    spec = eval_runs.load_eval_run_spec(RUN_SPEC_PATH)

    assert spec.plan_id == "oracle_signal_getattr_default_probe_matrix"
    assert len(spec.cases) == 1
    case = spec.cases[0]
    assert case.case_id == "signal_getattr_default_probe"
    assert case.task_path == "evals/tasks/oracle_signal_getattr_default_probe.json"
    assert case.query == QUERY
    assert case.budgets == PROBE_BUDGETS
    assert case.providers == PROBE_PROVIDERS


def test_getattr_default_probe_fixture_uses_minimal_lookup_outcome_payload_shape() -> (
    None
):
    """The fixture preserves the outcome-only payload for default-return probes."""
...
def test_getattr_default_probe_assets_stay_internal() -> None:
    ...
    assert "oracle_signal_getattr_default_probe" not in context_ir.__all__
    assert not hasattr(context_ir, "oracle_signal_getattr_default_probe")


def test_getattr_default_probe_default_subprocess_provider_owns_runtime_provenance(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The exact provider replays the default-return branch through subprocess."""
    setup = setup_eval_oracle_task(TASK_PATH)
    ...
    assert request_replay_fields_by_key["attribute_name"] == "missing_attribute"
...
def test_getattr_default_probe_default_subprocess_provider_runs_via_run_spec_name(
    tmp_path: Path,
) -> None:
    """Temporary run-spec dispatch preserves default-return subprocess provenance."""
    spec_path = _single_provider_run_spec_path(
        tmp_path,
        provider_name=DEFAULT_LOCAL_PYTHON_SUBPROCESS_PROVIDER,
        budget=100,
    )
    ledger_path = tmp_path / "getattr_default_probe_default_subprocess.jsonl"
    ...
    assert "missing_attribute" not in json.dumps(unsupported_unit, sort_keys=True)


def test_getattr_default_probe_run_executes_with_runtime_backed_raw_fields(
    tmp_path: Path,
) -> None:
    """Run execution preserves additive runtime provenance in raw pilot records."""
    ledger_path = tmp_path / "getattr_default_probe.jsonl"
    ...
    records = _parsed_ledger_records(ledger_path)
    assert execution.plan_id == "oracle_signal_getattr_default_probe_matrix"
