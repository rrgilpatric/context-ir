...
def _write_dynamic_import_program(tmp_path: Path) -> None:
    """Write a semantic fixture with one attachable dynamic import boundary."""
    (tmp_path / "main.py").write_text(
        textwrap.dedent(
            """
            import importlib
...
def _compile_result_omitting(unit_id: str, *, query: str) -> SemanticCompileResult:
    """Return a minimal prior compile result that omitted one unit."""
    return SemanticCompileResult(
        document="# Context",
        optimization=SemanticOptimizationResult(
            selections=(),
            omitted_unit_ids=(unit_id,),
            warnings=(),
            total_tokens=0,
            budget=64,
            confidence=0.0,
...
def test_diagnose_semantic_miss_plans_runtime_probe_for_omitted_attachable_unsupported(
    ...
    """Omitted attachable unsupported diagnostics expose a planned probe request."""
...
def test_diagnose_semantic_miss_skips_non_attachable_and_heuristic_boundaries(
    tmp_path: Path,
) -> None:
    """Unsupported non-attachable and heuristic diagnostics stay requestless."""
    unsupported_dir = tmp_path / "unsupported"
    unsupported_dir.mkdir()
    (unsupported_dir / "main.py").write_text(
        textwrap.dedent(
            """
            def run() -> None:
                getattr()
    ...
    unsupported_program = _semantic_program(unsupported_dir)
    unsupported_id = _unsupported_id_for(unsupported_program, "getattr()")

    unsupported_result = diagnose_semantic_miss(
        _compile_result_omitting(unsupported_id, query="getattr"),
        SemanticMissEvidence(
            kind=SemanticMissKind.ABSENT_SYMBOL,
            evidence="getattr()",
        ),
    ...
    frontier_dir.mkdir()
    _write_sample_program(frontier_dir)
    frontier_program = _semantic_program(frontier_dir)
    frontier_id = _frontier_id_for(frontier_program, "missing_call")

    frontier_result = diagnose_semantic_miss(
        _compile_result_omitting(frontier_id, query="missing call"),
        SemanticMissEvidence(
            kind=SemanticMissKind.ABSENT_SYMBOL,
            evidence="missing_call",
        ),
...
def test_recompile_semantic_context_carries_planned_requests_without_runtime_mutation(
    tmp_path: Path,
) -> None:
    """Recompile exposes diagnostic planned requests without runtime side effects."""
    _write_dynamic_import_program(tmp_path)
    program = _semantic_program(tmp_path)
    unsupported_id = _unsupported_id_for(program, "importlib.import_module(name)")
    previous_result = _compile_result_omitting(
        unsupported_id,
        query="dynamic import",
    )
    unsupported_before = list(program.unsupported_constructs)
    provenance_before = list(program.provenance_records)
    diagnostics_before = list(program.diagnostics)
    previous_optimization = previous_result.optimization
    previous_document = previous_result.document
    previous_context = previous_result.compile_context
    ...
    assert program.unsupported_constructs == unsupported_before
    assert program.provenance_records == provenance_before
    assert program.diagnostics == diagnostics_before
    assert previous_result.optimization == previous_optimization
    assert previous_result.document == previous_document
    assert previous_result.compile_context == previous_context
