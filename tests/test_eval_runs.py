...
GETATTR_PROBE_BUDGETS = (220, 100)
GETATTR_DEFAULT_PROBE_BUDGETS = (220, 100)
GETATTR_DEFAULT_VALUE_PROBE_BUDGETS = (220, 100)
...
def test_execute_eval_run_spec_populates_runtime_backed_fields_for_getattr_probe(
    tmp_path: Path,
) -> None:
    """The ``getattr`` pilot keeps runtime-backed fields additive under spec v1."""
    ...
    parsed_records = _parsed_ledger_records(ledger_path)
    assert execution.record_count == len(PROBE_PROVIDERS) * len(GETATTR_PROBE_BUDGETS)
    assert len(parsed_records) == len(PROBE_PROVIDERS) * len(GETATTR_PROBE_BUDGETS)
    assert {
        (record["provider_name"], record["budget"]) for record in parsed_records
    } == {
        (provider_name, budget)
        for provider_name in PROBE_PROVIDERS
        for budget in GETATTR_PROBE_BUDGETS
    }
...
def test_execute_eval_run_spec_populates_runtime_fields_for_getattr_default_probe(
    tmp_path: Path,
) -> None:
    """The defaulted ``getattr`` pilot keeps runtime fields additive under spec v1."""
