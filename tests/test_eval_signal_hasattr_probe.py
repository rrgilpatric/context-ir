...
RUNTIME_PAYLOAD = (("attribute_present", "true"),)
...
def _single_provider_run_spec_path(
    ...
    *,
    ...
    """Return a temporary hasattr probe run spec for one provider and budget."""
    spec_record = cast(
        dict[str, object],
    ...
    spec_path = tmp_path / "hasattr_probe_single_provider.json"
...
def test_hasattr_probe_default_subprocess_provider_owns_runtime_provenance(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The exact provider replays hasattr through subprocess-owned provenance."""
    setup = setup_eval_oracle_task(TASK_PATH)
...
def test_hasattr_probe_default_subprocess_provider_fails_closed_for_other_tasks() -> (
    None
):
...
def test_hasattr_probe_default_subprocess_provider_runs_via_run_spec_name(
    tmp_path: Path,
) -> None:
    ...
    ledger_path = tmp_path / "hasattr_probe_default_subprocess.jsonl"
    ...
    assert all(
        provenance_record["normalized_payload"] == {"attribute_present": "true"}
        for provenance_record in runtime_provenance_records
    )
...
def test_hasattr_probe_run_executes_with_runtime_backed_raw_fields(
    ...
    metrics_220 = cast(dict[str, object], record_220["metrics"])
    unsupported_unit_220 = next(
        unit
    ...
    metrics_100 = cast(dict[str, object], record_100["metrics"])
    unsupported_unit = next(
        unit
    ...
    assert record_220["provider_name"] == eval_providers.CONTEXT_IR_PROVIDER
    assert UNSUPPORTED_UNIT_ID in cast(list[str], record_220["selected_unit_ids"])
    assert metrics_220["uncertainty_honesty"] == 1.0
    assert unsupported_unit_220["primary_capability_tier"] == "unsupported/opaque"
    assert unsupported_unit_220["has_attached_runtime_provenance"] is True
    assert cast(
        list[str],
        unsupported_unit_220["attached_runtime_provenance_record_ids"],
    )

    assert record_100["spec_version"] == "v1"
    assert record_100["provider_name"] == eval_providers.CONTEXT_IR_PROVIDER
    assert UNSUPPORTED_UNIT_ID in cast(list[str], record_100["selected_unit_ids"])
    assert metrics_100["uncertainty_honesty"] == 1.0
    assert unsupported_unit["primary_capability_tier"] == "unsupported/opaque"
    assert unsupported_unit["has_attached_runtime_provenance"] is True
    assert cast(
        list[str],
