...
def test_exact_getattr_probe_appends_request_replay_payload_fields() -> None:
    """The exact name-variable getattr pilot appends pre-observation replay inputs."""
...
def test_exact_getattr_probe_fails_closed_for_wrong_fixture_snapshot(
    repository_snapshot_basis: RepositorySnapshotBasis,
) -> None:
...
def test_exact_dir_probe_appends_request_replay_payload_fields() -> None:
    """The exact one-argument dir pilot appends the deterministic object input."""
...
def test_exact_vars_type_error_probe_appends_request_replay_payload_fields() -> None:
    """The exact vars TypeError pilot appends the deterministic object input."""
...
def test_exact_vars_type_error_probe_fails_closed_for_wrong_snapshot(
    repository_snapshot_basis: RepositorySnapshotBasis,
) -> None:
...
def test_exact_dir_probe_fails_closed_for_wrong_snapshot(
    repository_snapshot_basis: RepositorySnapshotBasis,
) -> None:
    """The one-argument dir contract requires its exact clean fixture snapshot."""
    source_request = _reflective_dir_exact_replay_input_request()
    runner_request = _local_python_runner_request(
        request=source_request,
        repository_snapshot_basis=repository_snapshot_basis,
...
def test_exact_name_setattr_probe_appends_request_replay_payload_fields() -> None:
    """The exact name-variable setattr pilot is clean-fixture scoped."""
...
def test_exact_name_setattr_probe_fails_closed_for_wrong_fixture_snapshot(
    repository_snapshot_basis: RepositorySnapshotBasis,
) -> None:
    """The name-variable setattr contract requires its exact clean snapshot."""
    source_request = _runtime_mutation_setattr_name_exact_replay_input_request()
    runner_request = _local_python_runner_request(
        request=source_request,
        repository_snapshot_basis=repository_snapshot_basis,
...
def test_default_local_python_subprocess_runner_executes_exact_getattr_replay_inputs(
    tmp_path: Path,
) -> None:
    """The default runner observes the exact name-variable getattr replay."""
    project_source_path = str(Path(__file__).resolve().parents[1] / "src")
    (tmp_path / "main.py").write_text(
        textwrap.dedent(
            """
...
def test_default_local_python_subprocess_runner_rejects_exact_getattr_wrong_snapshot(
    ...
    """The exact name-variable getattr replay fails closed for a wrong snapshot."""
    project_source_path = str(Path(__file__).resolve().parents[1] / "src")
    (tmp_path / "main.py").write_text(
        textwrap.dedent(
            """
...
def test_reflective_vars_exact_local_python_runner_executes_type_error_subprocess(
    tmp_path: Path,
) -> None:
    """The composed helper replays ``main.probe_namespace(1)`` for exact vars."""
    project_source_path = str(Path(__file__).resolve().parents[1] / "src")
    (tmp_path / "main.py").write_text(
...
def test_reflective_vars_exact_local_python_runner_fails_closed_for_wrong_snapshot(
    repository_snapshot_basis: RepositorySnapshotBasis,
    tmp_path: Path,
) -> None:
    """Wrong or dirty vars TypeError snapshots fail before exact replay."""
    project_source_path = str(Path(__file__).resolve().parents[1] / "src")
    (tmp_path / "main.py").write_text(
        ("def probe_namespace(obj: object) -> object:\n    return vars(obj)\n"),
        encoding="utf-8",
...
def test_default_local_python_subprocess_runner_executes_exact_name_setattr(
    tmp_path: Path,
) -> None:
    """The default runner calls exact name-setattr with object, name, and value."""
    project_source_path = str(Path(__file__).resolve().parents[1] / "src")
    (tmp_path / "main.py").write_text(
        textwrap.dedent(
            """
    ...
    request = _runtime_mutation_setattr_name_exact_replay_input_request()
    runner_request = _local_python_runner_request(
        (
            _field("repository_root", str(tmp_path)),
            _field("working_directory", str(tmp_path)),
