"""Isolated ``delattr`` runtime-backed eval pilot tests."""
...
FIXTURE_ROOT = REPO_ROOT / "evals" / "fixtures" / "oracle_signal_delattr_probe"
TASK_PATH = REPO_ROOT / "evals" / "tasks" / "oracle_signal_delattr_probe.json"
RUN_SPEC_PATH = (
...
QUERY = (
    "Fix probe_delete_attribute unsupported delattr(obj, name) deleted attribute "
...
def test_delattr_probe_task_resolves_expected_selectors_deterministically() -> None:
    """The isolated delattr probe resolves the intended selectors."""
    setup = setup_eval_oracle_task(TASK_PATH)

    assert setup.task.task_id == "oracle_signal_delattr_probe"
    assert setup.task.fixture_id == "oracle_signal_delattr_probe"
    ...
    assert [resolved.resolved_unit_id for resolved in setup.resolved_selectors] == [
        "def:main.py:main.probe_delete_attribute",
...
def test_delattr_probe_assets_stay_internal() -> None:
    """The isolated delattr probe does not widen public exports."""
    ...
    assert "oracle_signal_delattr_probe" not in context_ir.__all__
    assert not hasattr(context_ir, "oracle_signal_delattr_probe")


def test_delattr_probe_default_local_provider_replays_exact_fixture(
    ...
    result = eval_providers.build_context_ir_default_local_python_subprocess_pack(
        eval_providers.EvalProviderRequest(
            ...
            task_id="oracle_signal_delattr_probe",
            query=QUERY,
            budget=220,
        )
    )
    ...
    assert result.task_id == "oracle_signal_delattr_probe"
    ...
    assert planned_request.boundary_text == "delattr(obj, name)"
    assert (
    ...
    assert planned_request.replay_target_seed == "main.probe_delete_attribute"
...
def test_delattr_probe_default_local_provider_fails_closed() -> None:
    ...
    with pytest.raises(ValueError) as exc_info:
        eval_providers.build_context_ir_default_local_python_subprocess_pack(
            eval_providers.EvalProviderRequest(
                repo_root=FIXTURE_ROOT,
                task_id="oracle_signal_delattr_probe_typo",
                query=QUERY,
                budget=220,
            )
        )
...
def test_delattr_probe_run_executes_with_additive_runtime_provenance(
    ...
    assert execution.plan_id == "oracle_signal_delattr_probe_matrix"
...
def test_delattr_probe_summary_surfaces_internal_capability_accounting(
    ...
    runtime_outcome_aggregate = next(
        ...
        and aggregate.payload_value == "deleted_attribute"
    )
