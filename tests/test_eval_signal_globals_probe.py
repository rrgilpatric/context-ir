"""Isolated ``globals()`` runtime-backed eval pilot tests."""
...
FIXTURE_ROOT = REPO_ROOT / "evals" / "fixtures" / "oracle_signal_globals_probe"
TASK_PATH = REPO_ROOT / "evals" / "tasks" / "oracle_signal_globals_probe.json"
RUN_SPEC_PATH = (
    REPO_ROOT / "evals" / "run_specs" / "oracle_signal_globals_probe_matrix.json"
)
...
QUERY = (
    "Fix probe_global_namespace unsupported globals() returned namespace and keep "
    "digest output aligned"
)
...
def _single_provider_run_spec_path(
    ...
    *,
    ...
    """Return a temporary globals-probe run spec for one provider and budget."""
    spec_record = cast(
        dict[str, object],
    ...
    spec_path = tmp_path / "globals_probe_single_provider.json"
...
def test_globals_probe_task_resolves_expected_selectors_deterministically() -> None:
    """The isolated globals probe resolves one edit and one unsupported selector."""
    setup = setup_eval_oracle_task(TASK_PATH)

    assert setup.task.task_id == "oracle_signal_globals_probe"
    assert setup.task.fixture_id == "oracle_signal_globals_probe"
    assert len(setup.task.expected_selectors) == 2
    assert isinstance(setup.task.expected_selectors[0], SymbolOracleSelector)
    assert isinstance(setup.task.expected_selectors[1], UnsupportedOracleSelector)
    assert [resolved.resolved_unit_id for resolved in setup.resolved_selectors] == [
        "def:main.py:main.probe_global_namespace",
        UNSUPPORTED_UNIT_ID,
    ]

    selector = setup.task.expected_selectors[1]
    unsupported = setup.resolved_selectors[1]
...
def test_globals_probe_run_spec_loads_budget_matrix() -> None:
    """The globals probe run spec is one task x two budgets x three providers."""
    spec = eval_runs.load_eval_run_spec(RUN_SPEC_PATH)

    assert spec.plan_id == "oracle_signal_globals_probe_matrix"
    assert len(spec.cases) == 1
    case = spec.cases[0]
    assert case.case_id == "signal_globals_probe"
    assert case.task_path == "evals/tasks/oracle_signal_globals_probe.json"
    assert case.query == QUERY
    assert case.budgets == PROBE_BUDGETS
    assert case.providers == PROBE_PROVIDERS


def test_globals_probe_fixture_uses_namespace_payload() -> None:
    """The fixture preserves the eval-only ``globals()`` runtime payload."""
    source = (FIXTURE_ROOT / "main.py").read_text(encoding="utf-8")
    observations = load_fixture_globals_runtime_observations(FIXTURE_ROOT)

    assert source.count("globals()") == 1
    assert len(observations) == 1
    assert observations[0].site.snippet == "globals()"
...
def test_globals_probe_assets_stay_internal() -> None:
    """The isolated globals probe does not widen public exports."""
    ...
    assert "oracle_signal_globals_probe" not in context_ir.__all__
    assert not hasattr(context_ir, "oracle_signal_globals_probe")


def test_globals_probe_default_subprocess_provider_owns_runtime_provenance(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The exact provider replays globals() through subprocess-owned provenance."""
    setup = setup_eval_oracle_task(TASK_PATH)
    ...
    provider_result = (
        eval_providers.build_context_ir_default_local_python_subprocess_pack(
            eval_providers.EvalProviderRequest(
                ...
                task_id="oracle_signal_globals_probe",
                query=QUERY,
                budget=100,
            )
        )
    ...
    observed_result = recompile_response.runner_attempt_collection.result_batch.results[
        0
    ]
    ...
    assert planned_request.form_label == "runtime_mutation:globals/0"
    assert planned_request.boundary_text == "globals()"
    ...
    assert (
        recompile_response.runner_attempt_collection.attempts[0].request
        is planned_request
    )
...
def test_globals_probe_default_subprocess_provider_fails_closed_for_other_tasks() -> (
    None
):
...
def test_globals_probe_default_subprocess_provider_runs_via_run_spec_name(
    tmp_path: Path,
) -> None:
    ...
    ledger_path = tmp_path / "globals_probe_default_subprocess.jsonl"
...
def test_globals_probe_run_executes_with_additive_runtime_provenance(
    tmp_path: Path,
) -> None:
    """Run execution preserves unsupported primary truth plus runtime support."""
    ledger_path = tmp_path / "globals_probe.jsonl"
    ...
    records = _parsed_ledger_records(ledger_path)
    assert execution.plan_id == "oracle_signal_globals_probe_matrix"
...
def test_globals_probe_summary_surfaces_internal_capability_accounting(
    tmp_path: Path,
) -> None:
