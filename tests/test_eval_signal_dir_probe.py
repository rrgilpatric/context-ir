"""Isolated one-argument ``dir`` runtime-backed eval pilot tests."""
...
FIXTURE_ROOT = REPO_ROOT / "evals" / "fixtures" / "oracle_signal_dir_probe"
TASK_PATH = REPO_ROOT / "evals" / "tasks" / "oracle_signal_dir_probe.json"
RUN_SPEC_PATH = (
    REPO_ROOT / "evals" / "run_specs" / "oracle_signal_dir_probe_matrix.json"
)
...
QUERY = (
    "Fix probe_directory unsupported dir(obj) listing and keep digest output aligned"
)
UNSUPPORTED_UNIT_ID = "unsupported:call:main.py:2:11"
RUNTIME_PAYLOAD = (("listing_entry_count", "74"),)
...
def _single_provider_run_spec_path(
    ...
    *,
    ...
    """Return a temporary dir/1 probe run spec for one provider and budget."""
    spec_record = cast(
        dict[str, object],
    ...
    spec_path = tmp_path / "dir_probe_single_provider.json"
...
def test_dir_probe_task_resolves_expected_selectors_deterministically() -> None:
    """The isolated one-argument probe resolves the intended selectors."""
    setup = setup_eval_oracle_task(TASK_PATH)

    assert setup.task.task_id == "oracle_signal_dir_probe"
    assert setup.task.fixture_id == "oracle_signal_dir_probe"
...
def test_dir_probe_run_spec_loads_cleanly_through_runner() -> None:
    """The isolated one-argument probe run spec stays valid runner input."""
    spec = eval_runs.load_eval_run_spec(RUN_SPEC_PATH)

    assert spec.plan_id == "oracle_signal_dir_probe_matrix"
    assert len(spec.cases) == 1
    case = spec.cases[0]
    assert case.case_id == "signal_dir_probe"
    assert case.task_path == "evals/tasks/oracle_signal_dir_probe.json"
    assert case.query == QUERY
    assert case.budgets == PROBE_BUDGETS
    assert case.providers == PROBE_PROVIDERS


def test_dir_probe_fixture_uses_one_argument_durable_payload_shape() -> None:
    """The fixture preserves the eval-only one-argument ``dir(obj)`` branch."""
    source = (FIXTURE_ROOT / "main.py").read_text(encoding="utf-8")
    observations = load_fixture_dir_runtime_observations(FIXTURE_ROOT)

    assert source.count("dir(obj)") == 1
    assert "dir()" not in source
    assert len(observations) == 1
    assert observations[0].site.snippet == "dir(obj)"
...
def test_dir_probe_assets_stay_internal() -> None:
    """The isolated one-argument probe does not widen public exports."""
    ...
    assert "oracle_signal_dir_probe" not in context_ir.__all__
    assert not hasattr(context_ir, "oracle_signal_dir_probe")


def test_dir_probe_default_subprocess_provider_owns_runtime_provenance(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The exact provider replays dir(obj) through subprocess-owned provenance."""
    setup = setup_eval_oracle_task(TASK_PATH)
    ...
    provider_result = (
        eval_providers.build_context_ir_default_local_python_subprocess_pack(
            eval_providers.EvalProviderRequest(
                ...
                task_id="oracle_signal_dir_probe",
                query=QUERY,
                budget=100,
            )
        )
    ...
    assert planned_request.form_label == "reflective_builtin:dir/1"
    assert planned_request.boundary_text == "dir(obj)"
    ...
    assert (
        provenance_record.evidence_origin is EvidenceOriginKind.RUNTIME_PROBE_IDENTITY
    )
...
def test_dir_probe_default_subprocess_provider_fails_closed_for_other_tasks() -> None:
    ...
    assert "oracle_signal_dir_probe" in message
    assert "oracle_signal_hasattr_probe" in message
    assert "oracle_signal_hasattr_false_probe" in message
    assert "oracle_signal_hasattr_literal_probe" in message
    assert "oracle_signal_getattr_probe" in message
    assert "oracle_signal_getattr_attribute_error_probe" in message
    assert "oracle_signal_getattr_default_probe" in message
    assert "oracle_signal_getattr_default_value_probe" in message
    assert "oracle_signal_getattr_literal_probe" in message
...
def test_dir_probe_default_subprocess_provider_runs_via_run_spec_name(
    tmp_path: Path,
) -> None:
    """Temporary run-spec dispatch preserves dir/1 subprocess provenance."""
    spec_path = _single_provider_run_spec_path(
        tmp_path,
        provider_name=DEFAULT_LOCAL_PYTHON_SUBPROCESS_PROVIDER,
        budget=100,
    )
    ledger_path = tmp_path / "dir_probe_default_subprocess.jsonl"
    ...
    assert all(
        provenance_record["normalized_payload"] == {"listing_entry_count": "74"}
        for provenance_record in runtime_provenance_records
    )
...
def test_dir_probe_run_executes_with_additive_runtime_provenance(
    tmp_path: Path,
) -> None:
    """Run execution preserves unsupported primary truth plus runtime support."""
    ledger_path = tmp_path / "dir_probe.jsonl"
    ...
    records = _parsed_ledger_records(ledger_path)
    assert execution.plan_id == "oracle_signal_dir_probe_matrix"
