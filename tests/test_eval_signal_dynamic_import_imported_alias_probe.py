"""Imported-alias ``load_module(name)`` runtime-backed eval pilot tests."""
...
TASK_PATH = (
    ...
    / "evals"
    / "tasks"
    / "oracle_signal_dynamic_import_imported_alias_probe.json"
)
RUN_SPEC_PATH = (
    REPO_ROOT
    / "evals"
...
def test_dynamic_import_imported_alias_probe_task_resolves_expected_selectors() -> None:
    """The imported-alias probe resolves intended symbol and boundary selectors."""
    setup = setup_eval_oracle_task(TASK_PATH)

    assert setup.task.task_id == "oracle_signal_dynamic_import_imported_alias_probe"
    assert setup.task.fixture_id == "oracle_signal_dynamic_import_imported_alias_probe"
...
def test_dynamic_import_imported_alias_probe_run_spec_loads_two_budget_matrix() -> None:
    """The run spec stays at 1 task x 2 budgets x 3 providers."""
    spec = eval_runs.load_eval_run_spec(RUN_SPEC_PATH)

    assert spec.plan_id == "oracle_signal_dynamic_import_imported_alias_probe_matrix"
    assert len(spec.cases) == 1
    case = spec.cases[0]
    assert case.case_id == "signal_dynamic_import_imported_alias_probe"
...
def test_dynamic_import_imported_alias_probe_fixture_uses_only_alias_shape() -> None:
    """The fixture preserves only the imported-alias dynamic import shape."""
    ...
    aliased_import_module_imports = [
        ...
        and node.module == "importlib"
        and any(
            alias.name == "import_module" and alias.asname == "load_module"
            for alias in node.names
        )
    ]
    alias_calls = [
    ...
    assert source.count("from importlib import import_module as load_module") == 1
    assert source.count('name = "plugins.weather"') == 1
    assert source.count("load_module(name)") == 1
    assert "import importlib" not in source
    assert "from importlib import import_module\n" not in source
    ...
    assert plain_import_module_imports == []
    assert len(aliased_import_module_imports) == 1
    assert len(alias_calls) == 1
    assert len(alias_calls[0].args) == 1
    assert alias_calls[0].keywords == []
    assert isinstance(alias_calls[0].args[0], ast.Name)
    assert alias_calls[0].args[0].id == "name"
    ...
    assert observations[0].site.snippet == "load_module(name)"
    ...
    assert observations[0].site.span.end_column == 30
    assert tuple(
...
def test_dynamic_import_imported_alias_probe_assets_stay_internal() -> None:
    """The imported-alias probe remains internal and does not widen public exports."""
    ...
    assert "oracle_signal_dynamic_import_imported_alias_probe" not in context_ir.__all__
    assert not hasattr(context_ir, "oracle_signal_dynamic_import_imported_alias_probe")


def test_imported_alias_probe_keeps_runtime_module_out_of_static_proof() -> None:
...
def test_imported_alias_probe_default_local_provider_replays_exact_fixture(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The default local-Python provider admits only this imported-alias fixture."""
    ...
    assert result.task_id == "oracle_signal_dynamic_import_imported_alias_probe"
    ...
    assert planned_request.source_site.span.end_column == 30
    assert planned_request.source_site.snippet == "load_module(name)"
    assert planned_request.boundary_text == "load_module(name)"
    assert (
        planned_request.family_label is eval_providers.RuntimeProbeFamily.DYNAMIC_IMPORT
    )
    assert planned_request.form_label == "dynamic_import:load_module/1"
...
def test_imported_alias_probe_default_local_provider_fails_closed_for_non_220_budget(
    monkeypatch: pytest.MonkeyPatch,
    budget: int,
) -> None:
...
def test_imported_alias_probe_run_executes_with_additive_runtime_provenance(
    tmp_path: Path,
) -> None:
    """Context IR attaches runtime provenance while primary truth stays opaque."""
    ledger_path = tmp_path / "dynamic_import_imported_alias_probe.jsonl"
    ...
    assert (
        execution.plan_id == "oracle_signal_dynamic_import_imported_alias_probe_matrix"
    )
