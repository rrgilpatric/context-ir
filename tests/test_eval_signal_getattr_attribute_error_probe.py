"""Isolated ``getattr`` raised-AttributeError runtime-backed eval pilot tests."""
...
FIXTURE_ROOT = (
    REPO_ROOT / "evals" / "fixtures" / "oracle_signal_getattr_attribute_error_probe"
)
TASK_PATH = (
    REPO_ROOT / "evals" / "tasks" / "oracle_signal_getattr_attribute_error_probe.json"
)
RUN_SPEC_PATH = (
    REPO_ROOT
    / "evals"
...
RUNTIME_PAYLOAD = (("lookup_outcome", "raised_attribute_error"),)
...
def _single_provider_run_spec_path(
    ...
    *,
    ...
    """Return a temporary attribute-error probe run spec for one provider."""
    spec_record = cast(
        dict[str, object],
    ...
    spec_path = tmp_path / "getattr_attribute_error_single_provider.json"
...
def _load_fixture_module() -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location(
        "oracle_signal_getattr_attribute_error_probe_fixture",
...
def test_getattr_attribute_error_probe_resolves_expected_selectors() -> None:
    """The raised-AttributeError probe resolves the intended unsupported selector."""
    setup = setup_eval_oracle_task(TASK_PATH)

    assert setup.task.task_id == "oracle_signal_getattr_attribute_error_probe"
    assert setup.task.fixture_id == "oracle_signal_getattr_attribute_error_probe"
...
def test_getattr_attribute_error_probe_run_spec_loads_cleanly() -> None:
    """The raised-AttributeError probe run spec stays valid runner input."""
    spec = eval_runs.load_eval_run_spec(RUN_SPEC_PATH)

    assert spec.plan_id == "oracle_signal_getattr_attribute_error_probe_matrix"
    assert len(spec.cases) == 1
    case = spec.cases[0]
    assert case.case_id == "signal_getattr_attribute_error_probe"
    assert (
        case.task_path == "evals/tasks/oracle_signal_getattr_attribute_error_probe.json"
    )
...
def test_getattr_attribute_error_probe_fixture_uses_raise_lookup_outcome() -> None:
    """The fixture covers the raise branch for two-argument ``getattr``."""
...
def test_getattr_attribute_error_probe_fixture_digest_is_deterministic() -> None:
    """The fixture catches ``AttributeError`` before rendering its digest."""
    module = _load_fixture_module()
    render_probe_digest = cast(Callable[[], str], module.render_probe_digest)

    assert render_probe_digest() == "getattr_attribute_error:raised_attribute_error"


def test_getattr_attribute_error_probe_assets_stay_internal() -> None:
    """The raised-AttributeError probe does not widen public exports."""
    ...
    assert "oracle_signal_getattr_attribute_error_probe" not in context_ir.__all__
    assert not hasattr(context_ir, "oracle_signal_getattr_attribute_error_probe")


def test_getattr_attribute_error_default_subprocess_provider_owns_runtime_provenance(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The exact provider replays raised AttributeError through subprocess."""
    setup = setup_eval_oracle_task(TASK_PATH)
    ...
    provider_result = (
        eval_providers.build_context_ir_default_local_python_subprocess_pack(
            eval_providers.EvalProviderRequest(
                ...
                task_id="oracle_signal_getattr_attribute_error_probe",
                query=QUERY,
                budget=100,
            )
        )
    ...
    assert recompile_request.diagnostic.planned_runtime_probe_requests == plan.requests
    ...
    assert planned_request.boundary_text == "getattr(obj, name)"
    ...
    assert (
        _json_replay_field_map(subprocess_stdin_payloads[0])["attribute_name"]
        == "definitely_missing_attribute"
    )
    assert request_replay_fields_by_key["object_type"] == "builtins.int"
    assert (
        request_replay_fields_by_key["attribute_name"] == "definitely_missing_attribute"
    )
    ...
    assert "observed_replay_inputs" not in provenance_detail
...
def test_getattr_attribute_error_default_subprocess_provider_runs_via_run_spec_name(
    tmp_path: Path,
) -> None:
    """Temporary run-spec dispatch preserves AttributeError subprocess provenance."""
    spec_path = _single_provider_run_spec_path(
        tmp_path,
        provider_name=DEFAULT_LOCAL_PYTHON_SUBPROCESS_PROVIDER,
        budget=100,
    )
    ledger_path = tmp_path / "getattr_attribute_error_default_subprocess.jsonl"
    ...
    assert "observed_replay_inputs" not in json.dumps(
...
def test_getattr_attribute_error_default_subprocess_provider_fails_closed() -> None:
    """Wrong task IDs remain unsupported by the exact subprocess provider."""
    with pytest.raises(
        ValueError,
        match="context_ir_default_local_python_subprocess only supports",
    ):
        eval_providers.build_context_ir_default_local_python_subprocess_pack(
            eval_providers.EvalProviderRequest(
                repo_root=FIXTURE_ROOT,
                task_id="oracle_signal_getattr_attribute_error_probe_typo",
                query=QUERY,
                budget=100,
            )
        )
...
def test_getattr_attribute_error_probe_run_preserves_additive_runtime_fields(
    tmp_path: Path,
) -> None:
    """Run execution keeps raised-AttributeError provenance additive only."""
    ledger_path = tmp_path / "getattr_attribute_error_probe.jsonl"
    ...
    records = _parsed_ledger_records(ledger_path)
    assert execution.plan_id == "oracle_signal_getattr_attribute_error_probe_matrix"
...
def test_getattr_attribute_error_probe_summary_keeps_runtime_additive(
    ...
    runtime_outcome_aggregate = next(
        ...
        and aggregate.payload_value == "raised_attribute_error"
    )
