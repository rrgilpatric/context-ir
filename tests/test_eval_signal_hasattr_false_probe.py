"""Isolated ``hasattr`` missing-attribute eval pilot tests."""
...
FIXTURE_ROOT = REPO_ROOT / "evals" / "fixtures" / "oracle_signal_hasattr_false_probe"
TASK_PATH = REPO_ROOT / "evals" / "tasks" / "oracle_signal_hasattr_false_probe.json"
RUN_SPEC_PATH = (
    REPO_ROOT / "evals" / "run_specs" / "oracle_signal_hasattr_false_probe_matrix.json"
)
...
RUNTIME_PAYLOAD = (("attribute_present", "false"),)
...
def _single_provider_run_spec_path(
    ...
    *,
    ...
    """Return a temporary false-branch run spec for one provider and budget."""
    spec_record = cast(
        dict[str, object],
    ...
    spec_path = tmp_path / "hasattr_false_probe_single_provider.json"
...
def _replay_field_map(
    fields: tuple[runtime_probe_results.RuntimeProbeReplayField, ...],
) -> dict[str, str]:
    """Return replay fields keyed by field name."""
    return {field.key: field.value for field in fields}


def _json_replay_field_map(payload: dict[str, object]) -> dict[str, str]:
    """Return replay fields from one serialized worker stdin payload."""
    fields = cast(list[dict[str, str]], payload["request_replay_payload_fields"])
    return {field["key"]: field["value"] for field in fields}


def _load_fixture_module() -> ModuleType:
    """Load the isolated fixture module so its false branch can be checked."""
    spec = importlib.util.spec_from_file_location(
        "oracle_signal_hasattr_false_probe_fixture",
...
def test_hasattr_false_probe_resolves_unsupported_primary_selector() -> None:
    """The primary durable selector is the unsupported ``hasattr`` boundary."""
    setup = setup_eval_oracle_task(TASK_PATH)

    assert setup.task.task_id == "oracle_signal_hasattr_false_probe"
    assert setup.task.fixture_id == "oracle_signal_hasattr_false_probe"
    assert len(setup.task.expected_selectors) == 3
    assert isinstance(setup.task.expected_selectors[0], UnsupportedOracleSelector)
    ...
    unsupported = setup.resolved_selectors[0]
...
def test_hasattr_false_probe_run_spec_loads_cleanly() -> None:
    """The false-branch probe run spec stays at 1 task x 2 budgets x 3 providers."""
    spec = eval_runs.load_eval_run_spec(RUN_SPEC_PATH)

    assert spec.plan_id == "oracle_signal_hasattr_false_probe_matrix"
    assert len(spec.cases) == 1
    case = spec.cases[0]
    assert case.case_id == "signal_hasattr_false_probe"
    assert case.task_path == "evals/tasks/oracle_signal_hasattr_false_probe.json"
    assert case.query == QUERY
    assert case.budgets == PROBE_BUDGETS
    assert case.providers == PROBE_PROVIDERS


def test_hasattr_false_probe_fixture_uses_missing_attribute_payload() -> None:
    """The fixture preserves the exact ``hasattr(obj, name)`` false payload."""
    source = (FIXTURE_ROOT / "main.py").read_text(encoding="utf-8")
    observations = load_fixture_hasattr_runtime_observations(FIXTURE_ROOT)

    assert source.count("hasattr(obj, name)") == 1
    assert len(observations) == 1
    assert observations[0].site.snippet == "hasattr(obj, name)"
    ...
    assert tuple(
        ...
        ("attribute_name", MISSING_ATTRIBUTE_NAME),
    )
    assert (
...
def test_hasattr_false_probe_fixture_digest_is_deterministic() -> None:
    """The fixture executes the missing-attribute branch."""
    module = _load_fixture_module()
    render_probe_digest = cast(Callable[[], str], module.render_probe_digest)

    assert render_probe_digest() == "hasattr_false:missing"


def test_hasattr_false_probe_assets_stay_internal() -> None:
    """The false-branch probe does not widen public exports."""
    ...
    assert "oracle_signal_hasattr_false_probe" not in context_ir.__all__
    assert not hasattr(context_ir, "oracle_signal_hasattr_false_probe")


def test_hasattr_false_probe_default_subprocess_provider_owns_runtime_provenance(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The exact provider replays false-branch hasattr through subprocess proof."""
    setup = setup_eval_oracle_task(TASK_PATH)
    ...
    subprocess_stdin_payloads: list[dict[str, object]] = []
    ...
    def spying_run(*args: object, **kwargs: object) -> object:
        ...
        if isinstance(raw_input, str):
            ...
            subprocess_stdin_payloads.append(cast(dict[str, object], payload))
    ...
    provider_result = (
        eval_providers.build_context_ir_default_local_python_subprocess_pack(
            eval_providers.EvalProviderRequest(
                ...
                task_id="oracle_signal_hasattr_false_probe",
                query=QUERY,
                budget=100,
            )
        )
    ...
    assert len(recompile_responses) == 1
    ...
    request_replay_fields = attempts[0].execution_input.replay_artifact.replay_inputs
    request_replay_fields_by_key = _replay_field_map(request_replay_fields)
    ...
    assert planned_request.form_label == "reflective_builtin:hasattr/2"
    assert planned_request.boundary_text == "hasattr(obj, name)"
    assert planned_request.replay_target_seed == "main.probe_attribute"
    ...
    assert len(subprocess_stdin_payloads) == 1
    assert _json_replay_field_map(subprocess_stdin_payloads[0])["object_type"] == (
        "builtins.int"
    )
    assert _json_replay_field_map(subprocess_stdin_payloads[0])["attribute_name"] == (
        MISSING_ATTRIBUTE_NAME
    )
    assert request_replay_fields_by_key["object_type"] == "builtins.int"
    assert request_replay_fields_by_key["attribute_name"] == MISSING_ATTRIBUTE_NAME
    assert attempts[0].observed_replay_inputs == ()
    ...
    assert observed_result.replay_artifact.replay_inputs == request_replay_fields
    assert (
...
def test_hasattr_false_probe_default_subprocess_provider_runs_via_run_spec_name(
    tmp_path: Path,
) -> None:
    """Temporary run-spec dispatch preserves false-branch subprocess provenance."""
    spec_path = _single_provider_run_spec_path(
        tmp_path,
        provider_name=DEFAULT_LOCAL_PYTHON_SUBPROCESS_PROVIDER,
        budget=100,
    )
    ledger_path = tmp_path / "hasattr_false_probe_default_subprocess.jsonl"
    ...
    assert all(
        provenance_record["normalized_payload"] == {"attribute_present": "false"}
        for provenance_record in runtime_provenance_records
    )
    assert MISSING_ATTRIBUTE_NAME not in json.dumps(unsupported_unit, sort_keys=True)


def test_hasattr_false_probe_run_keeps_runtime_additive_and_baselines_empty(
    tmp_path: Path,
) -> None:
    """Run execution keeps false-branch provenance additive and internal."""
    ledger_path = tmp_path / "hasattr_false_probe.jsonl"
    ...
    records = _parsed_ledger_records(ledger_path)
    assert execution.plan_id == "oracle_signal_hasattr_false_probe_matrix"
    ...
    for budget in PROBE_BUDGETS:
        for provider_name in BASELINE_PROVIDERS:
            ...
            assert baseline_record["selected_files"] == []
        ...
        resolved_selectors = _resolved_selectors(record)
        unsupported_selector = next(
            selector
        ...
        assert record["selected_unit_ids"] == [UNSUPPORTED_UNIT_ID]
        assert len(selected_units) == 1
        [unsupported_unit] = selected_units
        assert unsupported_unit["unit_id"] == UNSUPPORTED_UNIT_ID
        assert metrics["uncertainty_honesty"] == 1.0
