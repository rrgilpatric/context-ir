"""Root ``importlib.import_module(name)`` runtime-backed eval pilot tests."""
...
FIXTURE_ROOT = (
    REPO_ROOT / "evals" / "fixtures" / "oracle_signal_dynamic_import_root_probe"
)
TASK_PATH = (
    REPO_ROOT / "evals" / "tasks" / "oracle_signal_dynamic_import_root_probe.json"
)
RUN_SPEC_PATH = (
    REPO_ROOT
    / "evals"
...
QUERY = (
    "Fix unsupported dynamic import importlib.import_module(name) "
...
def test_dynamic_import_root_probe_task_resolves_expected_selectors() -> None:
    """The root importlib probe resolves the intended symbol and boundary selectors."""
    setup = setup_eval_oracle_task(TASK_PATH)

    assert setup.task.task_id == "oracle_signal_dynamic_import_root_probe"
    assert setup.task.fixture_id == "oracle_signal_dynamic_import_root_probe"
...
def test_dynamic_import_root_probe_run_spec_loads_two_budget_matrix() -> None:
    """The run spec stays at 1 task x 2 budgets x 3 providers."""
    spec = eval_runs.load_eval_run_spec(RUN_SPEC_PATH)

    assert spec.plan_id == "oracle_signal_dynamic_import_root_probe_matrix"
    assert len(spec.cases) == 1
    case = spec.cases[0]
    assert case.case_id == "signal_dynamic_import_root_probe"
    assert case.task_path == "evals/tasks/oracle_signal_dynamic_import_root_probe.json"
    assert case.query == QUERY
    assert case.budgets == PROBE_BUDGETS
    assert case.providers == PROBE_PROVIDERS


def test_dynamic_import_root_probe_fixture_uses_only_root_importlib_shape() -> None:
    """The fixture preserves the root-module ``importlib.import_module(name)`` case."""
    ...
    assert source.count("importlib.import_module(name)") == 1
    ...
    assert isinstance(root_importlib_calls[0].args[0], ast.Name)
    assert root_importlib_calls[0].args[0].id == "name"
    ...
    assert observations[0].site.snippet == "importlib.import_module(name)"
...
def test_dynamic_import_root_probe_assets_stay_internal() -> None:
    """The root importlib probe remains internal and does not widen public exports."""
    ...
    assert "oracle_signal_dynamic_import_root_probe" not in context_ir.__all__
    assert not hasattr(context_ir, "oracle_signal_dynamic_import_root_probe")


def test_dynamic_import_root_probe_keeps_imported_module_out_of_static_edges() -> None:
...
def test_dynamic_import_root_probe_default_local_provider_replays_exact_fixture(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The default local-Python provider admits only this root-name fixture."""
    ...
    result = eval_providers.build_context_ir_default_local_python_subprocess_pack(
        eval_providers.EvalProviderRequest(
            ...
            task_id="oracle_signal_dynamic_import_root_probe",
            query=QUERY,
            budget=220,
        )
    )
    ...
    assert result.task_id == "oracle_signal_dynamic_import_root_probe"
    ...
    assert planned_request.source_site.span.end_column == 42
    assert planned_request.source_site.snippet == "importlib.import_module(name)"
    assert planned_request.boundary_text == "importlib.import_module(name)"
    assert (
...
def test_root_probe_default_local_provider_fails_closed_for_non_220_budget(
    monkeypatch: pytest.MonkeyPatch,
    budget: int,
) -> None:
...
def test_dynamic_import_root_probe_run_executes_with_additive_runtime_provenance(
    tmp_path: Path,
) -> None:
    """Context IR attaches runtime provenance while primary truth stays opaque."""
    ledger_path = tmp_path / "dynamic_import_root_probe.jsonl"
    ...
    assert execution.plan_id == "oracle_signal_dynamic_import_root_probe_matrix"
