"""Isolated defaulted ``getattr`` value-return eval pilot tests."""
...
FIXTURE_ROOT = (
    REPO_ROOT / "evals" / "fixtures" / "oracle_signal_getattr_default_value_probe"
)
TASK_PATH = (
    REPO_ROOT / "evals" / "tasks" / "oracle_signal_getattr_default_value_probe.json"
)
RUN_SPEC_PATH = (
    REPO_ROOT
    / "evals"
...
QUERY = (
    "Fix probe_attribute unsupported getattr(obj, name, default) value-return branch "
    "and keep digest output aligned"
)
UNSUPPORTED_UNIT_ID = "unsupported:call:main.py:2:11"
RUNTIME_PAYLOAD = (("lookup_outcome", "returned_value"),)
...
def _single_provider_run_spec_path(
    ...
    *,
    ...
    """Return a temporary defaulted getattr value run spec for one provider."""
    spec_record = cast(
        dict[str, object],
    ...
    spec_path = tmp_path / "getattr_default_value_probe_single_provider.json"
...
def test_getattr_default_value_probe_resolves_expected_selectors() -> None:
    """The value-return probe resolves the intended unsupported selector."""
    setup = setup_eval_oracle_task(TASK_PATH)

    assert setup.task.task_id == "oracle_signal_getattr_default_value_probe"
    assert setup.task.fixture_id == "oracle_signal_getattr_default_value_probe"
...
def test_getattr_default_value_probe_run_spec_loads_cleanly() -> None:
    """The value-return probe run spec stays valid runner input."""
    spec = eval_runs.load_eval_run_spec(RUN_SPEC_PATH)

    assert spec.plan_id == "oracle_signal_getattr_default_value_probe_matrix"
    assert len(spec.cases) == 1
    case = spec.cases[0]
    assert case.case_id == "signal_getattr_default_value_probe"
    assert (
        case.task_path == "evals/tasks/oracle_signal_getattr_default_value_probe.json"
    )
...
def test_getattr_default_value_probe_fixture_uses_value_lookup_outcome() -> None:
    """The fixture covers the value-return branch for defaulted ``getattr``."""
...
def test_getattr_default_value_probe_assets_stay_internal() -> None:
    """The value-return probe remains internal and does not widen public exports."""
    ...
    assert "oracle_signal_getattr_default_value_probe" not in context_ir.__all__
    assert not hasattr(context_ir, "oracle_signal_getattr_default_value_probe")


def test_getattr_default_value_probe_default_subprocess_provider_owns_runtime_provenance(  # noqa: E501
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The exact provider replays the value-return branch through subprocess."""
    setup = setup_eval_oracle_task(TASK_PATH)
    ...
    provider_result = (
        eval_providers.build_context_ir_default_local_python_subprocess_pack(
            eval_providers.EvalProviderRequest(
                ...
                task_id="oracle_signal_getattr_default_value_probe",
                query=QUERY,
                budget=100,
            )
        )
    ...
    assert recompile_request.miss_evidence == SemanticMissEvidence(
        ...
        evidence="getattr(obj, name, default)",
    )
    ...
    assert planned_request.form_label == "reflective_builtin:getattr/3"
    assert planned_request.boundary_text == "getattr(obj, name, default)"
    ...
    assert request_replay_fields_by_key["attribute_name"] == "bit_length"
...
def test_getattr_default_value_probe_default_subprocess_provider_runs_via_run_spec_name(
    tmp_path: Path,
) -> None:
    """Temporary run-spec dispatch preserves value-return subprocess provenance."""
    spec_path = _single_provider_run_spec_path(
        tmp_path,
        provider_name=DEFAULT_LOCAL_PYTHON_SUBPROCESS_PROVIDER,
        budget=100,
    )
    ledger_path = tmp_path / "getattr_default_value_probe_default_subprocess.jsonl"
    ...
    assert "bit_length" not in json.dumps(unsupported_unit, sort_keys=True)


def test_getattr_default_value_probe_run_preserves_runtime_backed_fields(
    tmp_path: Path,
) -> None:
    """Run execution keeps additive runtime provenance on the unsupported unit."""
    ...
    records = _parsed_ledger_records(ledger_path)
    assert execution.plan_id == "oracle_signal_getattr_default_value_probe_matrix"
...
def test_getattr_default_value_probe_summary_keeps_runtime_additive(
    ...
    """The probe renders tier-aware accounting without runtime-backed promotion."""
    ledger_path = tmp_path / "getattr_default_value_probe.jsonl"
    ...
    for markdown in (rendered, report.markdown_report):
        ...
        assert "| lookup_outcome | returned_value | 6 |" in markdown
