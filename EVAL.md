...
`oracle_signal_dynamic_import_probe_matrix` evidence is the original
...
internal eval-only
`oracle_signal_dynamic_import_builtins_attr_probe_matrix` evidence is narrow
`DYNAMIC_IMPORT` / builtins-attribute `builtins.__import__(name)` sibling
evidence. The current
internal eval-only `oracle_signal_dynamic_import_builtins_alias_probe_matrix`
evidence is narrow `DYNAMIC_IMPORT` / builtins-alias
`loader.__import__(name)` sibling evidence. The current
...
sibling evidence. The current internal eval-only
`oracle_signal_dynamic_import_imported_alias_probe_matrix` evidence is narrow
`DYNAMIC_IMPORT` / imported-alias `load_module(name)` sibling evidence. The
current internal eval-only
`oracle_signal_dynamic_import_root_alias_probe_matrix` evidence is narrow
...
`delattr(obj, "flag")`: 1 task x 2 budgets x 3 providers at budgets
`[220, 100]`, against providers `context_ir`, `lexical_top_k_files`, and
`import_neighborhood_files`; the same exact replay identity now also has
default local runtime probe acquisition. The fixture boundary is the
direct-literal call site only; it excludes a name variable,
`delattr(obj, name)`, `setattr`, other runtime-mutation forms, and generalized
runtime-mutation support. Runtime payload is exactly
...
budgets. File-level baselines may select `main.py` at budget `220`, but their
selected-unit layer remains empty and they do not select the
unsupported/runtime evidence unit. No static `flag` dependency edge, selected
attribute symbol, or selected `flag` unit is introduced. The no-widening
boundary preserves public comparative claims at the existing quad matrix and
includes no runtime acquisition widening beyond that exact replay identity,
API/MCP/package-export/schema/scoring/optimizer/compiler/winner-selection/product/public
...
evidence, the current internal eval-only
`DYNAMIC_IMPORT` / imported-name `import_module(name)` sibling evidence, the
current internal eval-only `DYNAMIC_IMPORT` / imported-alias
`load_module(name)` sibling evidence, the current
internal eval-only `DYNAMIC_IMPORT` / root-module alias
`loader.import_module(name)` sibling evidence, and the runtime-outcome
accounting do not widen public claims, public APIs, MCP behavior,
...
benchmark widening.
The current internal `DYNAMIC_IMPORT` original provider/budget evidence covers
only `oracle_signal_dynamic_import_probe_matrix`: 1 task x 3 budgets x 3
providers at budgets `[220, 180, 100]`, against providers `context_ir`,
...
Runtime provenance remains additive only. Winner behavior is budget-specific:
budget `100` winner is `context_ir`; budgets `180` and `220` winners remain
`import_neighborhood_files`. No `plugins.weather` dependency edge, selected
symbol, or selected `plugins/weather.py` unit is introduced, public
comparative claims remain bounded to the existing quad matrix, and no
public/API/MCP/package-export/schema/scoring/optimizer/compiler/winner-selection/product/public
benchmark widening or product-surface widening is included.
...
public comparative claims remain bounded to the existing quad matrix. It does
not cover root-module `importlib.import_module(name)` expansion, imported-name
`import_module(name)` expansion, imported-alias `load_module(name)` expansion,
literal dynamic import expansion, `__import__(name)`,
`builtins.__import__`, globals/locals/fromlist forms, namespace mutation,
generated-code dependency modeling, generalized dynamic import support,
public/API/MCP/package-export/schema/scoring/optimizer/compiler/winner-selection/product/public
benchmark widening, or product-surface widening.
...
public/API/MCP/package-export/schema/scoring/optimizer/compiler/winner-selection/product/public
benchmark widening, or product-surface widening.
The current internal `DYNAMIC_IMPORT` / builtins-attribute
`builtins.__import__(name)` sibling evidence covers only
...
existing quad matrix. It does not cover the separate builtins-alias
...
public/API/MCP/package-export/schema/scoring/optimizer/compiler/winner-selection/product/public
benchmark widening, or product-surface widening.
The current internal `DYNAMIC_IMPORT` / builtins-alias
`loader.__import__(name)` sibling evidence covers only
...
additive only, no dependency edge or selected symbol is created from
`plugins.weather`, and public comparative claims remain bounded to the
existing quad matrix. It does not cover other builtins alias names, unaliased
`builtins.__import__(name)` expansion in this matrix, bare `__import__(name)`
expansion, shadowed/rebound/non-builtins forms, wrong-arity forms, literal
`loader.__import__("plugins.weather")` expansion, fromlist/globals/locals
...
public/API/MCP/package-export/schema/scoring/optimizer/compiler/winner-selection/product/public
benchmark widening, or product-surface widening.
The current internal `DYNAMIC_IMPORT` / imported-alias `load_module(name)`
sibling evidence covers only
`oracle_signal_dynamic_import_imported_alias_probe_matrix`: 1 task x 2 budgets x 3 providers at budgets `[220, 100]`, against providers `context_ir`,
`lexical_top_k_files`, and `import_neighborhood_files`. The fixture boundary
...
additive only, no dependency edge or selected symbol is created from
`plugins.weather`, and public comparative claims remain bounded to the
existing quad matrix. It does not cover root-module
...
selected symbol is introduced, and no
source/runtime/API/MCP/package-export/schema/scoring/optimizer/compiler/winner-selection/product/public
...
only `oracle_signal_getattr_attribute_error_probe_matrix`: 1 task x 2 budgets
x 3 providers at budgets `220` and `100`, against providers `context_ir`,
`lexical_top_k_files`, and `import_neighborhood_files`. The fixture, task,
...
- Internal eval runtime-outcome accounting is supported for normalized runtime
  ...
  eval-only `DYNAMIC_IMPORT` / builtins-attribute
  `builtins.__import__(name)` sibling evidence records
  `imported_module=plugins.weather`, while the current internal
  eval-only `DYNAMIC_IMPORT` / builtins-alias `loader.__import__(name)`
  sibling evidence records `imported_module=plugins.weather`, while the current
  internal
  eval-only `DYNAMIC_IMPORT` / imported-name `import_module(name)` sibling
  evidence records `imported_module=plugins.weather`, and the current internal
  eval-only `DYNAMIC_IMPORT` / imported-alias `load_module(name)` sibling
  evidence records `imported_module=plugins.weather`, while the current internal eval-only `DYNAMIC_IMPORT` / root-module
  alias `loader.import_module(name)` sibling evidence records
  `imported_module=plugins.weather`, preserving existing tier/provider
...
- Internal runtime-backed evidence currently covers narrow pilots only:
  - the `DYNAMIC_IMPORT` internal provider/budget matrix for the
    `oracle_signal_dynamic_import_probe` task through
    `oracle_signal_dynamic_import_probe_matrix`: 1 task x 3 budgets x 3
    providers at budgets `[220, 180, 100]`, against providers `context_ir`,
    `lexical_top_k_files`, and `import_neighborhood_files`; runtime payload
    remains `imported_module=plugins.weather`; baseline providers remain
    empty at all budgets; budget `100` non-selection is explicit because
    `context_ir` selects only `def:main.py:main.load_weather_plugin`,
    `def:main.py:main.render_probe_digest`, and
    `frontier:call:main.py:6:11`, does not select
    `unsupported:call:main.py:5:13`, and does not select
    `plugins/weather.py`; the unsupported selector remains
    `unsupported/opaque` with `unsupported_reason_code`, `opaque_boundary`,
    and attached runtime provenance; runtime provenance remains additive only;
    budget `100` winner is `context_ir`, while budgets `180` and `220`
    winners remain `import_neighborhood_files`; no `plugins.weather`
    dependency edge, selected symbol, selected `plugins/weather.py` unit, or
    public/API/MCP/package-export/schema/scoring/optimizer/compiler/winner-selection/product/public
  ...
  - the narrow internal eval-only `DYNAMIC_IMPORT` / builtin
    ...
    `oracle_signal_dynamic_import_builtin_probe_matrix`
  - the current internal eval-only `DYNAMIC_IMPORT` / builtins-attribute
    `builtins.__import__(name)` sibling evidence for
    `oracle_signal_dynamic_import_builtins_attr_probe_matrix`
  - the current internal eval-only `DYNAMIC_IMPORT` / builtins-alias
    `loader.__import__(name)` sibling evidence for
  ...
  - the current internal eval-only `DYNAMIC_IMPORT` / imported-alias
    `load_module(name)` sibling evidence for
    `oracle_signal_dynamic_import_imported_alias_probe_matrix`
  - the current internal eval-only `DYNAMIC_IMPORT` /
    root-module alias `loader.import_module(name)` sibling evidence for
    `oracle_signal_dynamic_import_root_alias_probe_matrix`
...
- The current internal eval-only `DYNAMIC_IMPORT` / builtins-attribute
  ...
  only. The runtime payload is `imported_module=plugins.weather`; primary
  selector and selected-unit truth remain `unsupported/opaque`, runtime
  provenance remains additive only, no dependency edge or selected symbol is
  created from `plugins.weather`, and public comparative claims remain bounded
  to the existing quad matrix. This evidence does not cover builtins alias
  support, bare `__import__(name)` expansion, shadowed/rebound/aliased forms,
...
- The current internal eval-only `DYNAMIC_IMPORT` / builtins-alias
  ...
  is `import builtins as loader`, `name = "plugins.weather"`, and exactly
  `loader.__import__(name)`, with bounded `sys.modules[name]` retrieval only.
  The runtime payload is `imported_module=plugins.weather`; primary selector
  ...
  `plugins.weather`, and public comparative claims remain bounded to the
  existing quad matrix. This evidence does not cover other builtins alias
...
- The current internal eval-only `DYNAMIC_IMPORT` / imported-name
  ...
  benchmark widening.
- The current internal eval-only `DYNAMIC_IMPORT` / imported-alias
  `load_module(name)` sibling evidence covers only
  `oracle_signal_dynamic_import_imported_alias_probe_matrix`: 1 task x 2 budgets x 3 providers at budgets `[220, 100]`, against providers `context_ir`,
  `lexical_top_k_files`, and `import_neighborhood_files`. The fixture boundary
  is `from importlib import import_module as load_module`,
  `name = "plugins.weather"`, and exactly `load_module(name)`. The runtime
  ...
  additive only, no dependency edge or selected symbol is created from
  `plugins.weather`, and public comparative claims remain bounded to the
  existing quad matrix. This evidence does not cover root-module
  `importlib.import_module(name)` expansion, imported-name `import_module(name)`
  expansion, literal `load_module("plugins.weather")` expansion,
  `loader.import_module(name)`, `__import__(name)`, `builtins.__import__`,
  globals/locals/fromlist forms, namespace mutation, generated-code dependency
  modeling, generalized dynamic import support, or
  public/API/MCP/package-export/schema/scoring/optimizer/compiler/winner-selection/product/public
  benchmark widening.
...
- The current internal eval-only `EXEC_OR_EVAL` /
  ...
  generated-code dependency edge or selected symbol is introduced, and public
  comparative claims remain bounded to the existing quad matrix. It does not
...
- Runtime-backed task families beyond the current narrow `DYNAMIC_IMPORT`
  ...
  sibling evidence, the narrow builtin `__import__(name)`
  sibling evidence, the current builtins-attribute
  `builtins.__import__(name)` sibling evidence, the current builtins-alias
  `loader.__import__(name)` sibling evidence, the current imported-name
  `import_module(name)` sibling
...
- Narrow runtime-backed internal pilots: `DYNAMIC_IMPORT`, including the
  ...
  current imported-name `import_module(name)` sibling evidence, plus the
  current imported-alias `load_module(name)` sibling evidence, plus the
  current root-module alias `loader.import_module(name)` sibling evidence, plus
...
- Narrow builtin `__import__(name)` provider/budget evidence: the narrow
  ...
  only `oracle_signal_dynamic_import_builtin_probe_matrix`: 1 task x 2 budgets x 3 providers at budgets `[220, 100]`, against providers `context_ir`,
  `lexical_top_k_files`, and `import_neighborhood_files`. The fixture boundary
  is `name = "plugins.weather"` and exactly `__import__(name)`, with bounded
  `sys.modules[name]` retrieval only. The runtime payload is
  `imported_module=plugins.weather`; primary selector and selected-unit truth
  ...
  public/API/MCP/package-export/schema/scoring/optimizer/compiler/winner-selection/product/public
  benchmark widening, and product-surface widening.
- Narrow builtins-attribute `builtins.__import__(name)` provider/budget
  evidence: the current internal eval-only `DYNAMIC_IMPORT` sibling evidence
  covers only `oracle_signal_dynamic_import_builtins_attr_probe_matrix`: 1 task
  ...
  remains additive only, no dependency edge or selected symbol is created from
  `plugins.weather`, and public comparative claims remain bounded to the
  existing quad matrix. The evidence excludes the separate builtins-alias
  `loader.__import__(name)` form, bare `__import__(name)` expansion,
  shadowed/rebound/aliased forms, wrong-arity forms, literal
  `builtins.__import__("plugins.weather")` expansion,
  fromlist/globals/locals forms, namespace mutation, generated-code dependency
  ...
  public/API/MCP/package-export/schema/scoring/optimizer/compiler/winner-selection/product/public
  benchmark widening, and product-surface widening.
- Narrow builtins-alias `loader.__import__(name)` provider/budget evidence:
  the current internal eval-only `DYNAMIC_IMPORT` sibling evidence covers only
  `oracle_signal_dynamic_import_builtins_alias_probe_matrix`: 1 task x 2 budgets x 3 providers at budgets `[220, 100]`, against providers `context_ir`,
  `lexical_top_k_files`, and `import_neighborhood_files`. The fixture boundary
  is `import builtins as loader`, `name = "plugins.weather"`, and exactly
  ...
  remains additive only, no dependency edge or selected symbol is created from
  `plugins.weather`, and public comparative claims remain bounded to the
  existing quad matrix. The evidence excludes other builtins alias names,
...
- Narrow imported-name `import_module(name)` provider/budget evidence: the
  ...
  public/API/MCP/package-export/schema/scoring/optimizer/compiler/winner-selection/product/public
  benchmark widening, and product-surface widening.
- Narrow imported-alias `load_module(name)` provider/budget evidence: the
  current internal eval-only `DYNAMIC_IMPORT` sibling evidence covers only
  `oracle_signal_dynamic_import_imported_alias_probe_matrix`: 1 task x 2 budgets x 3 providers at budgets `[220, 100]`, against providers `context_ir`,
  `lexical_top_k_files`, and `import_neighborhood_files`. The fixture boundary
  is `from importlib import import_module as load_module`,
  ...
  additive only, no dependency edge or selected symbol is created from
  `plugins.weather`, and public comparative claims remain bounded to the
  existing quad matrix. The evidence excludes root-module
  `importlib.import_module(name)` expansion, imported-name `import_module(name)`
  expansion, literal `load_module("plugins.weather")` expansion,
  `loader.import_module(name)`, `__import__(name)`, `builtins.__import__`,
  globals/locals/fromlist forms, namespace mutation, generated-code dependency
  ...
  public/API/MCP/package-export/schema/scoring/optimizer/compiler/winner-selection/product/public
  benchmark widening, and product-surface widening.
- Narrow root-module alias `loader.import_module(name)` provider/budget
  evidence: the current internal eval-only
  `DYNAMIC_IMPORT` sibling evidence covers only
  `oracle_signal_dynamic_import_root_alias_probe_matrix`: 1 task x 2 budgets x 3 providers at budgets `[220, 100]`, against providers `context_ir`,
  `lexical_top_k_files`, and `import_neighborhood_files`. The fixture boundary
  is `import importlib as loader`, `name = "plugins.weather"`, and exactly
  `loader.import_module(name)`. The runtime payload is
...
- Narrow raised-`AttributeError` `getattr(obj, name)` provider/budget
  evidence: the current internal eval-only
  `oracle_signal_getattr_attribute_error_probe_matrix` covers only 1 task x 2
  budgets x 3 providers at budgets `220` and `100`, against providers
  `context_ir`, `lexical_top_k_files`, and `import_neighborhood_files`, with
  unchanged fixture, task, query, and runtime payload across both budget rows;
  fixture boundary exactly `getattr(obj, name)`, caught `AttributeError` for
  ...
  selected symbol created, and public comparative claims bounded to the
...
- Narrow `exec(source)` provider/budget evidence: the current
  ...
  mutation modeling is added, and public comparative claims remain bounded to
  the existing quad matrix.
- Narrow original `DYNAMIC_IMPORT` provider/budget evidence: the current
  internal eval-only evidence covers only
  `oracle_signal_dynamic_import_probe_matrix`: 1 task x 3 budgets x 3
  providers at budgets `[220, 180, 100]`, against providers `context_ir`,
  `lexical_top_k_files`, and `import_neighborhood_files`. The fixture, task,
  query, and runtime payload remain unchanged; runtime payload remains
  ...
  is `context_ir`; budgets `180` and `220` winners remain
  `import_neighborhood_files`. No `plugins.weather` dependency edge, selected
...
- Internal runtime-backed eval evidence currently covers the narrow
  ...
  task x 2 budgets x 3 providers at budgets `220` and `100`, against
  providers `context_ir`, `lexical_top_k_files`, and
  `import_neighborhood_files`; fixture, task, query, and runtime payload remain
  unchanged across both budget rows; fixture boundary is exactly
  `getattr(obj, name)`, with `AttributeError` caught so
  ...
  covers only `oracle_signal_setattr_probe_matrix`: 1 task x 2 budgets x 3
  providers at budgets `[220, 100]`, against providers `context_ir`,
  `lexical_top_k_files`, and `import_neighborhood_files`, with runtime payload
  `mutation_outcome=returned_none`; fixture, task, query, and runtime payload
  remain unchanged across both budget rows; selector/runtime-mutation surface
  ...
  provenance remains additive only; baseline providers remain empty at both
  budgets; and public comparative claims remain bounded to the existing quad
  matrix. The current internal eval-only
  ...
  attaches only to the preserved `EXEC_OR_EVAL` unsupported finding for
  `eval(source)`. Baseline providers remain empty at both budgets; at both
  budgets `context_ir` selects the same unsupported/opaque boundary unit with
  additive runtime provenance; and no generated-code dependency edge or
  selected symbol is introduced. The current internal eval-only `EXEC_OR_EVAL` /
  `exec(source)` evidence covers only `oracle_signal_exec_probe_matrix`: 1
  task x 2 budgets x 3 providers at budgets `[220, 100]`, against providers
  `context_ir`, `lexical_top_k_files`, and `import_neighborhood_files`, with
  fixture/call boundary `source = "pass"` and exactly `exec(source)`, executed
  source parsing as exactly one `ast.Pass`, runtime proof boundary
  `execution_outcome=completed`, `source_shape=literal_statement`, valid
  `source_sha256` for exact `"pass"`, and non-empty
  `durable_payload_reference`; optional `statement_kind=pass` is additive
  summary only, and runtime provenance attaches only to the preserved
  `EXEC_OR_EVAL` unsupported finding for `exec(source)`. Baseline providers
  remain empty at both budgets; at both budgets `context_ir` selects the same
  unsupported/opaque boundary unit with additive runtime provenance; and no
  generated-code dependency edge or selected symbol is introduced. The current
  internal
  ...
  public comparative claims remain bounded to the existing quad matrix.
  The current internal eval-only `DYNAMIC_IMPORT` /
  root-module alias `loader.import_module(name)` sibling evidence covers only
  `oracle_signal_dynamic_import_root_alias_probe_matrix`: 1 task x 2 budgets x 3 providers at budgets `[220, 100]`, against providers `context_ir`,
  `lexical_top_k_files`, and `import_neighborhood_files`; fixture boundary
...
- No broad or public Python dynamic-semantics claims, including generalized
  ...
  `__import__(name)` sibling evidence, the builtins-attribute
  `builtins.__import__(name)` sibling evidence, the builtins-alias
  `loader.__import__(name)` sibling evidence, and the imported-name
  `import_module(name)` sibling evidence, and the imported-alias
  `load_module(name)` sibling evidence, plus the current root-module alias
  ...
  The current internal eval-only `DYNAMIC_IMPORT` / builtins-attribute
  `builtins.__import__(name)` sibling matrix does not add generalized dynamic
  import support and does not cover the separate builtins-alias
  `loader.__import__(name)` form, bare `__import__(name)` expansion,
  shadowed/rebound/aliased forms, wrong-arity forms, literal
  `builtins.__import__("plugins.weather")` expansion,
  fromlist/globals/locals forms, namespace mutation, or generated-code
  dependency modeling.
  The current internal eval-only `DYNAMIC_IMPORT` / builtins-alias
  `loader.__import__(name)` sibling matrix does not add generalized dynamic
  import support and does not cover other builtins alias names, unaliased
  ...
  The current internal eval-only `DYNAMIC_IMPORT` / imported-alias
  `load_module(name)` sibling matrix does not add generalized dynamic import
  support and does not cover root-module `importlib.import_module(name)`
  expansion, imported-name `import_module(name)` expansion, literal
  `load_module("plugins.weather")` expansion, `loader.import_module(name)`,
  `__import__(name)`, `builtins.__import__`, globals/locals/fromlist forms,
  namespace mutation, or generated-code dependency modeling.
  The current internal eval-only `DYNAMIC_IMPORT` /
  root-module alias `loader.import_module(name)` sibling matrix does not add
  generalized dynamic import support and does not cover root-module
...
2. Broaden hybrid static + runtime analysis evidence beyond the current narrow
   ...
   builtin `__import__(name)` sibling evidence, plus the current
   builtins-attribute `builtins.__import__(name)` sibling evidence,
   ...
   `eval(source)` evidence, plus the current internal eval-only
   `EXEC_OR_EVAL` / `exec(source)` evidence,
   plus the current internal eval-only
   `DYNAMIC_IMPORT` / root-module alias `loader.import_module(name)` sibling
   evidence,
